 ...
 *
 * 예전엔 여섯 축이 전부 4로 똑같아서, 패스 길이를 한 칸 옮기는 것과 수비 라인을
 * 한 칸 올리는 것이 같은 대가를 냈다.
 */
const AXIS_COST = {
 ...
 *
 * 예전엔 40을 깔았다("프로 팀은 이만큼은 소화한다"). 하지만 그러면 전술을 계속
 * 갈아엎는 감독과 하나를 파고드는 감독의 바닥이 같아진다. 전술을 모르는 상태는
 * 실제로 존재하고, 그게 0이다.
 *
...
export function familiarityForSetup(
   ...
   * ⚠️ **해 본 전술이면 그 기억만 쓴다.**
   *
   * 예전엔 전이 후보와 `max`로 섞었다. 개인 거리가 도입되면서 그 거리가 음수가
   * 될 수 있게 되자(자기 축구로 가는 변경) **다른 전술에서 유추한 값이 실제로
   * 도달했던 값보다 커지는** 일이 생겼고, A↔B를 오갈 때마다 그렇게 부풀어 왕복이
   * 새기 시작했다(70 → 75 → …). 해 봤으면 그때 값이 진실이다.
   */
