 ...
 *
 * 순서가 곧 크기다(적대 → 신뢰). ⚠️ **가운데가 없다** — 칸이 짝수인 것이 규칙이다.
 * 등급을 고르는 것은 모델이라, 「아직 아무 일도 없다」를 뜻하는 가운데 칸이 있으면 어느
 * 쪽으로 기울지 정하지 않고도 답을 낼 수 있다. 가운데에 가장 가까운 답은 `distant`와
 * `cordial`이고, 둘 다 이미 한쪽으로 기울어 있다.
 *
