 ...
 *
 * 전력 비는 `exp(ABILITY_LOG_SLOPE × 평점 차)`라(§1.1) 1.2는 평점 **10점** 차다 —
 * 전술 한 판이 흔들 수 있는 폭(`TACTIC_SWING`)의 바깥이라, 지시로 뒤집을 수 없는
 * 격차가 이 이름을 받는다.
 */
const EDGE_BIG_RATIO = 1.2;
/** 이 전력비부터 "뚜렷한 우위" — 평점 5점 차. 지시로 좁힐 수는 있어도 닫지는 못한다 */
const EDGE_CLEAR_RATIO = 1.1;
 ...
 * 벌어지는 폭도 함께 바뀐다. 눈금을 옮겼으면 `zone-baseline` 하네스의 등급 분포로
 * 다시 맞춘다 — 세 크기가 각각 몫을 갖고 「압도적」이 가장 드물어야 한다.
 */
