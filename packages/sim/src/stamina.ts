...
/**
 * 전술이 만드는 소모 배율 (0.84 ~ 1.16).
 *
 * 압박이 가장 비싸다 — 게겐프레싱이 로테이션 없이 한 시즌을 못 버티는 이유다.
 * 라인을 올리면 뒤로 돌아가는 회복 주행이 늘고, 폭을 넓게 쓰면 측면 자원이
 * 좌우로 더 많이 커버한다.
 *
 * **폭은 실제 팀 간 주행량의 폭에서 온다.** 1부의 팀 평균 총 주행거리는
 * 104\~115km(선수당 9.5\~10.5km)로 스타일이 갈라 놓는 폭이 ±5%뿐이고, 고강도
 * 주행은 그보다 훨씬 크게(±30%) 갈린다. 피로는 그 둘 사이를 재므로 **네 축을
 * 끝까지 민 양 끝이 ±16%**다. 한 축(압박)만으로는 ±8%다.
 *
 * ⚠️ **위끝은 구멍 문턱(`GAP_THRESHOLD`)과의 거리가 정한다.** 기본 전술로 풀타임을
 * 뛴 중앙 미드필더에게 남는 여유는 지구력 70에서 30 대 22, 곧 활동량으로 11점뿐이다
 * (기준 활동량 45의 25%). 전술이 그보다 많이 얹히면 **체력 100으로 나선 선발이
 * 지시 한 줄에 통째로 구멍에 걸린다** — 대가가 아니라 스위치다. 그래서 이 배율은
 * 지구력 축과 같은 눈금으로 맞춰 두었다: **소모에 닿는 셋(압박·템포·라인)을 끝까지
 * 올리는 것 = 지구력이 스무 칸 낮은 선수를 세우는 것**(지구력 70 중원이 24를 남겨
 * 기본 전술의 지구력 50과 같은 자리에 선다). 감독이 외울 수 있는 크기이면서
 * 벤치 다섯으로 답할 수 있는 크기다.
 */
const TACTICAL_DRAIN_MIN = 0.84;
const TACTICAL_DRAIN_MAX = 1.16;

/**
 * 축을 한 칸 올릴 때 팀 전체 소모에 얹히는 몫 — 압박이 가장 비싸다.
 *
 * 넷의 합(0.08)이 한쪽 끝까지의 폭이라 위 clamp와 정확히 맞물린다 — 잘리는 구간이
 * 없으므로 한 축을 만지면 그만큼만 움직인다.
 */
 ...
 *
 * ⚠️ **이건 나눠 주는 저울이지 위에 얹는 몫이 아니다.** 팀이 무는 총량은
 * `tacticalDrain` 하나가 정하고, 여기서는 그 총량이 열한 명에게 어떻게 갈리는지만
 * 정한다 — 그래서 압박의 가산(앞선·중원 여섯)과 감산(센터백·골키퍼 셋)이 XI 전체로는
 * 거의 상쇄된다(+1%). 감산이 가산의 두 배인 것도 그 머릿수 때문이다. 가산만 두면
 * 압박 한 칸이 `tacticalDrain`에서 한 번, 여기서 또 한 번 물려 **가장 많이 뛰는
 * 자리에서만 축의 값이 배로 서고**, 그 자리는 구멍 문턱에 가장 가까운 자리다.
 */
/** 폭 한 칸이 측면 자원에 더 얹는 몫 */
const WIDE_WIDTH_STEP = 0.03;
/** 압박 한 칸이 전방·중원에 더 얹는 몫 */
const PRESS_STEP_FRONT = 0.015;
 ...
 *
 * ⚠️ **전술을 끝까지 올린 판에서는 지구력이 그 예외를 고른다.** 압박·템포·라인을
 * 다 올려도 체력 100으로 나선 지구력 70 중원은 24를 남겨 문턱 밖에 서고(90은 31),
 * **지구력 60은 88분에, 50은 82분에 걸린다** — 그래서 "압박 축구를 하려면 뛸 수 있는
 * 선수가 필요하다"가 스쿼드를 고르는 판단이 된다. 지시 한 줄이 자리를 가리지 않고
 * 전원을 걸리게 하면 그건 선택이 아니라 스위치다 (`pnpm balance match-stamina`).
 */
...
export function fatigueFromMinutes(minutes: number, condition: number): number {
  ...
  return ((FATIGUE_PER_FULL_MATCH * minutes) / FULL_MATCH_MINUTES) * congestion;
}
