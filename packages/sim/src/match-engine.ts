 ...
 *
 * `temperScale`은 시트의 `temper`가 거는 배수다(`SidePacket.temper` — sim `sheet.ts`).
 * **상대 가중이라 팀 카드 총량은 움직이지 않는다**: 한 명을 낮추면 같은 장수가 팀
 * 동료에게 가고, 주는 것은 그 선수의 두 번째 경고다.
 */
...
function pickBooked(
  ...
  /** 선수 id → 시트의 `temper`가 거는 카드 배수. 없으면 전원 1 */
  temperScale?: ReadonlyMap<string, number>,
): Player | null {
  return weightedPick(rng, outfield(squad, gone), (p) =>
    bookingWeight(p, (booked?.[p.id] ?? 0) > 0, temperScale?.get(p.id) ?? 1),
  );
}
...
/**
 * **등급의 경계 — 리그 분포의 분위다** (player.md §5.3).
 *
 * 한 시즌(296일) 동안 전 구단 1군의 하루치 저울 123만 표본에서 85%가 62.6,
 * 95%가 87.5였다. 절대선이 아니라 **"이 리그에서 남들보다 위태로운가"**라, 세계가
 * 통째로 지치면 등급도 함께 움직인다. 등급별 실제 부상률이 이 순서로 서는지는
 * `pnpm balance injury-rate`가 잰다.
 */
 ...
 *
 * 셋을 다 짚으면 아무것도 짚지 않은 것과 같다. 4분의 1은 세 항 중 가장 큰 몫이
 * 언제나 넘는 선이기도 해서(합이 곧 들림이므로 최대 몫은 1/3 이상), 등급이 오른
 * 선수에게 원인이 하나도 서지 않는 일은 없다.
 */
...
/**
 * **감독이 미리 읽는 부상 위험** — 등급과 그 원인 (player.md §5.3).
 *
 * 새로 재는 값이 하나도 없다. 경기가 누가 다칠지 고를 때 쓰는 **그 저울**
 * (`injuryWeight`)을 그대로 읽어 낱말로 옮긴다 — 두 벌을 두면 화면이 「낮음」이라
 * 적은 선수가 코어의 굴림에서는 맨 앞에 선다.
 *
 * ⚠️ **굴림에는 닿지 않는다.** 등급은 읽는 쪽의 낱말이고, 누가 다치는지는 저울이
 * 그대로 정한다. 여기서 경계를 옮겨도 부상은 한 건도 움직이지 않는다.
 *
...
function causesFor(packet: StrengthPacket, side: MatchSide): PacketTag[] {
   ...
   * 순서는 매치업 → **시트** → 상성·구멍이다 (match.md §4) — 감독의 판독이 골로
   * 이어졌으면 그 문장이 근거로 선다.
   */
  const key =
    packet.keyPoints.find((tag) => tag.source === "sheet" && tag.favours === side) ??
    packet.keyPoints.find((tag) => tag.favours === side);
  if (key) return [key];
 ...
 *
 * 장부의 분은 마지막 사건의 것이라 굴린 시계와 어긋난다. 뒤에 서 있기도 하고
 * (사건 없이 흐른 자리) 앞에 서 있기도 하다 — 골 뒤로 밀린 분이 그렇다 (match.md
 * §1.4). 목표 분을 장부의 분에서 재면 그 차이가 그대로 구간의 길이를 깎는다.
 *
 * 그래서 **시계가 있으면 시계만 읽는다.** 밀린 분이 시계로 흘러 들어가면 골 하나마다
 * 최대 1분이 굴려지지 않은 채 사라져 슈팅 총량 계약이 깨진다. 장부의 분으로 돌아가는
 * 것은 이 값이 없는 **옛 세이브**뿐이다.
 */
export function segmentStartClock(ledger: MatchLedgerState, clock?: number): number {
  ...
  return Math.max(clock ?? ledger.minute, PHASE_START[phase]);
}
...
export function simulateSegment(input: SegmentInput): SegmentPlan {
  ...
  /**
   * **시트가 판 밖에서 닿는 두 곳** — 다리(`legs`)와 카드(`temper`) (match.md §1.6).
   *
   * 존 전력은 패킷이 이미 반영했고 이 둘은 패킷이 사람마다 배수로 들고 온다
   * (`SidePacket.legs`·`temper` — sim `sheet.ts`). 담기지 않은 선수는 1이라
   * **아무것도 바뀌지 않는다.** 시트가 없는 경기(간이 시뮬·옛 세이브)도 그렇다.
   */
  const drainOf = new Map<string, number>([
    ...Object.entries(packet.home.legs ?? {}),
    ...Object.entries(packet.away.legs ?? {}),
  ]);
  const bookingScaleOf = new Map<string, number>([
    ...Object.entries(packet.home.temper ?? {}),
    ...Object.entries(packet.away.temper ?? {}),
  ]);
  ...
  /**
   * **이 구간이 실을 수 있는 가장 이른 분** — 장부는 시간 역행을 반려한다.
   *
   * 하프의 첫 구간은 그 국면이 시작하는 분에서 출발한다 — 앞 하프가 규정 시각에
   * 끝나므로(`halfEnd`) 45·90·105가 그대로 다음 하프의 0분이다.
   *
   * 사건이 설 때마다 위로 움직인다(`stamp`). 굴린 시계보다 앞설 수 있고, 그것이
   * 골 뒤로 밀린 분이다 — 피로와 흐름의 양은 이 값이 아니라 시계로 잰다 (§1.4).
   */
  let minuteFloor = started ? Math.max(ledger.minute, PHASE_START[phase]) : 0;
  /**
  ...
  const addFatigue = (untilClock: number) => {
    const elapsed = Math.max(0, untilClock - clockFrom);
    if (elapsed <= 0) return;
    for (const side of ["home", "away"] as const) {
      /**
  ...
  const finish = (stop: SegmentStop, minute: number, clock: number): SegmentPlan => {
    ...
    /**
     * **피로도 흐름의 양도 장부의 분이 아니라 굴린 시계로 잰다** — 골 뒤로 밀린 분은
     * 시계를 움직이지 않으므로(match.md §1.4), 여기서 장부의 분을 읽으면 골 하나마다
     * 최대 1분어치의 다리와 패스가 더 세어진다.
     */
    addFatigue(clock);
    spreadFlow(Math.max(0, clock - clockFrom));
    return {
      events,
      stop,
      minute,
      // 다음 구간이 이어받을 자리 — **실제로 굴린 소수 시각 그대로다**
      clock,
      fatigue,
  ...
  const spreadFlow = (minutes: number) => {
    ...
    for (const side of ["home", "away"] as const) {
      ...
      const fouls = spreadCount(Math.round(sp.fouls * elapsedShare), players, (p) =>
        // 거칠기가 내려간 선수는 파울도 덜 한다 — 카드와 같은 가중이므로 여기서 함께 준다
  ...
  /**
   * **직전 골의 분** — 그 분은 그 골 혼자 갖는다. 골은 시간순이라 마지막 것만 본다.
   */
  let goalMinute = -1;
  for (const event of ledger.events) if (event.type === "goal") goalMinute = event.minute;
  /**
   * **연속 시계의 한 자리를 장부의 정수 분으로 옮긴다** (match.md §1.4).
   *
   * 규정 90분에는 재개까지의 시간이 없다 — 골이 들어가면 공은 센터서클로 돌아가고
   * 그 사이가 추가시간이 된다. 시계에 그 자리가 없으니 골 직후의 사건이 골과 같은
   * 분에 앉아 기록지에 「46′ 골, 46′ 골」이 연달아 찍힌다 (경기의 5.3%).
   *
   * 그래서 **골의 분과 겹치면 그다음 분**이다. 골 **앞**은 건드리지 않는다 —
   * 같은 분의 슛 → 선방 → 골은 한 장면이고 그렇게 읽혀야 한다. 골이 아닌 사건끼리
   * 같은 분에 서는 것도 그대로 둔다: 거기에 손을 대면 사건 간격에 바닥을 까는 것이라
   * 경기당 슈팅이 깎인다 (§1.4).
   *
   * ⚠️ **밀린 분은 시계를 밀지 않는다.** `t`도 `SegmentPlan.clock`도 그대로다.
   */
  const stamp = (at: number): number => {
    const m = Math.max(1, Math.floor(at), minuteFloor);
    minuteFloor = m === goalMinute ? m + 1 : m;
    return minuteFloor;
  };
  ...
  for (let guard = 0; guard < 60; guard++) {
    ...
    if (events.length >= MAX_SEGMENT_EVENTS) {
      ...
      return finish("flow", Math.max(last, minuteFloor), t);
    }
    /**
    ...
    if (next >= rollTo) {
      ...
      if (rollTo < halfEnd) {
        ...
        if (windowStop === "flow") return finish("flow", Math.max(last, minuteFloor), rollTo);
        /**
        ...
        return finish(
          "requested",
          Math.max(Math.floor(rollTo + CLOCK_EPSILON), last, minuteFloor),
          rollTo,
        );
      }
      /**
       * 장부는 시간 역행을 반려한다 — **이 구간이 이미 실은 분보다 이르면 안 된다.**
       * 짧게 부른 구간이 밀어 둔 시각도, 골 뒤로 밀린 분도 여기 들어 있다: 44′ 골 뒤의
       * 골이 45′를 받으면 그 하프의 휘슬은 46′다 (추가시간이 그 자리다, §1.4).
       */
      const minute = Math.max(halfEnd, minuteFloor);
      /**
    ...
    if (kind === "card") {
      ...
      /**
       * **곧장 퇴장만 절대량이 움직인다.** 누가 카드를 받는지는 상대 가중이라 총량이
       * 보존되지만, 이 굴림은 뽑힌 사람 하나의 확률이라 시트의 `temper`가 그대로
       * 곱해진다 — 거칠기가 내려간 선수가 걷어차서 나가는 일은 실제로 줄어든다.
       */
  ...
  return finish("flow", Math.max(minuteFloor, quiet), t);
}
