...
/**
 * 실행하지 않은 도구 호출을 닫는 말 — 어댑터 셋이 같은 문장을 쓴다.
 *
 * 잘린 응답의 호출은 인자가 끊겨 있어 돌리지 않는데(models.md §3), 짝 없는 호출이
 * 이력에 남으면 그 이력을 재사용하는 다음 요청이 제공자에게 거부된다.
 */
...
export interface TurnRequest {
  ...
  signal?: AbortSignal;
  /**
   * 도구 왕복 **하나**가 쓴 몫 — **계측이 넣는다**(`meterLlm`). 어댑터는 응답
   * 하나를 받아 누적하는 그 자리에서 이번 왕복의 delta를 부른다.
   *
   * ⚠️ 이 길이 없으면 실패로 끝난 호출의 토큰이 통째로 사라진다 — 결과가 돌아온
   * 뒤에만 적으면 여덟 번을 왕복하다 시한에 걸린 턴이 0으로 적힌다. 시한 예외는
   * 시한 래퍼가 만들고 어댑터의 예외는 경주에서 져 버려지므로, 예외에 실어
   * 보내는 길로는 그 자리를 덮을 수 없다 (models.md §4).
   */
  onUsage?: (delta: TurnUsage) => void;
}
