...
export const MATCH_STAMINA = defineHarness({
  ...
  bands: [
    ...
    { metric: "고압박 — 기준 풀타임 뒤 남은 체력 (지구력 70)", role: "guard", min: 22, unit: "score", why: "**이 하네스의 본론.** 압박·템포·라인을 끝까지 올리는 것은 감독이 가장 먼저 하는 지시이고, `tacticalDrain`(≤1.16)과 `positionalTacticWeight`의 압박 가산이 함께 얹히는 유일한 자리다. 그 판에서도 **체력 100으로 출발한 평범한 중원은 문턱 밖에 서야 한다** — 여기가 22 아래면 지시는 대가를 무는 선택이 아니라 전원을 구멍으로 보내는 스위치이고, 감독은 교체로 답할 수 없다. 실측 24는 우연이 아니라 눈금이다: 소모에 닿는 셋을 끝까지 올린 대가를 **지구력 스무 칸**에 맞췄으므로 맨 위의 「지구력 50·기본 전술」 값과 같은 자리에 선다 (match.md §3)" },
    { metric: "고압박 — 풀타임 뒤 남은 체력", role: "measure", unit: "score", why: "같은 스쿼드·같은 상대의 기본 전술 값과 나란히 읽는다 — 이 차이가 지시의 체력 대가다" },
    { metric: "고압박 — 풀타임에 구멍 난 선발 비율", role: "guard", max: 0.35, unit: "ratio", why: "지시의 대가는 있어야 하지만 **전원은 아니다.** 열한 중 서넛까지는 감독이 벤치 다섯으로 답할 수 있는 자리이고, 그 위로는 라인마다 `GAP_PENALTY`가 걸려 전력이 통째로 주저앉는다. 동시에 중계가 「다리가 멈췄다」로 채워져 문장이 뜻을 잃는다. **바닥을 걸지 않는 것도 뜻이 있다** — 위의 기준 케이스와 달리 이 팔의 선발은 체력 77쯤에서 킥오프하므로, 여기가 기본 팔(3%)과 붙으면 지시가 다리에 아무 값도 하지 않고 있다는 뜻이다" },
...
export const INJURY_RATE = defineHarness({
  ...
  doc: MATCH,
  cost: "간이 시뮬 72,000판 + 구간 시뮬 12,000판 · 3~4분",
  // prettier-ignore
  bands: [
