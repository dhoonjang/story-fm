...
export interface CupCatalogEntry {
   ...
   *
   * **실측 비율 기반·규모 압축** (competition.md §4.4). 실제 UEFA 2024-27 배분의
   * 비율을 따른다 — 대회 간 1 : 0.2 : 0.1, 승 : 무 = 3 : 1, 참가비 ≈ 승수당의
   * 8~9배. 절대 규모는 우리 경제에 맞춰 눌렀다: tier 1 구단의 시즌 중계권+스폰서
   * 수입이 약 £228m이고, UCL 우승 경로 총액이 그것의 3분의 1 정도(£64m)가 되게
   * 잡았다. 실제 배분의 절반 가까이는 우리가 모델링하지 않는 시장 가치 몫(value
   * pillar)이라 절대치를 그대로 옮기지 않는다. 수치 조정은 이 표만 고치면 된다.
   */
  prize: {
