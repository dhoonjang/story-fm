 ...
 * 맨유가 8월에 컵을 시작한다.
 *
 * 그래서 **날짜는 1부가 들어오는 실제 라운드의 것**을 쓴다 — 리그컵 9월 말, FA컵
 * 1월, 코파 델 레이 1월 초, 포칼 8월 말이다.
 *
 * **이름은 32클럽이 실제로 서는 라운드의 것**이다. 리그컵 3라운드(32팀)·코파
 * 32강(dieciseisavos, 32팀)·쿠프 32강(16es, 32팀)은 규모까지 실제와 같아 그대로고,
 * 실제 진입 라운드가 64팀인 FA컵과 포칼만 한 라운드 뒤의 이름으로 시작한다 —
 * FA컵은 4라운드(4·5라운드·8강·준결승·결승), 포칼은 2라운드(2라운드·16강·8강·
 * 준결승·결승)다. 이름을 진입 라운드 쪽에 맞추면 FA컵이 3라운드 다음에 8강으로
 * 건너뛰어 **5라운드가 통째로 없는 대회**가 되고, 포칼은 16강을 잃는다 — 규모를
 * 못 맞추는 것보다 그쪽이 크게 어긋난다 (competition.md §3.1).
 *
...
export const DOMESTIC_CUP_CATALOG_SEED: readonly DomesticCupEntry[] = [
  {
    ...
    twoLegged: [],
    drawStyle: "per-round",
    // 첫 라운드 추첨 12월 8일 → 경기 1월 10일 (실제 2025-26 3라운드의 날짜)
    firstDraw: [12, 8],
    // 다음 라운드 추첨은 **마지막 경기 직후** 그 경기장에서 (TNT Sports 생중계)
    drawDelayDays: 0,
    homeRule: "draw",
    // 1/10 → 2/7 → 8강(3/21) → 준결승(4/25, 웸블리) → 결승(5/16, 웸블리) —
    // 2025-26 실제 3·4라운드·8강·준결승·결승 날짜 그대로다. 32클럽이 서는 라운드가
    // 4·5라운드라 이름만 한 칸 뒤이고, 날짜는 리그 달력이 비워 둔 자리를 지킨다.
    windows: {
      league: [1, 10],
      playoff: [1, 10],
      r32: [1, 10],
    ...
    stageNames: { r32: "4라운드", r16: "5라운드" },
    europeanTicket: "uel",
    // FA 프라이즈 펀드 실측 — 4R 승 £120k · 5R 승 £225k · 준결승 진출 £450k ·
    // 우승 £2M · 준우승 £1M (라운드 상금은 그 라운드를 이겨 올라갈 때 받는다)
    prize: {
      round: { r16: 120_000, qf: 225_000, sf: 450_000 },
      winner: 2_000_000,
      runnerUp: 1_000_000,
    },
  },
  ...
  },
  {
    ...
    homeRule: "underdog",
    // 8월 말 2라운드 → 10월 말 16강 → 2월 8강 → 4월 준결승 → 5월 말 베를린.
  ...
  },
  {
     ...
     * 라운드인 **32강(16es, 1월 중순)** — 규모가 정확히 맞는 자리다 (FA컵·포칼도
     * 같은 규칙으로 이름을 정한다).
