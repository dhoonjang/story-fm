...
export const DOMESTIC_CUP_CATALOG_SEED: readonly DomesticCupEntry[] = [
  ...
  },
  {
    ...
    homeRule: "underdog",
    // 8월 말 1라운드 → 10월 말 2라운드 → 2월 8강 → 4월 준결승 → 5월 말 베를린.
    // 분데스리가 클럽이 실제로 1라운드부터 나오는 대회다.
    // (실제 12월 16강은 없다 — 우리 필드가 절반이라 2라운드 뒤가 곧 8강이다).
    // ⚠️ 실제 1라운드는 8/15~18, **분데스리가 개막 앞주**다 — 코파 이탈리아와 같은
    // 이유로 개막 뒤로 옮겼다 (`firstCupRoundFloor`, season.md §3).
    windows: {
      league: [8, 26],
      playoff: [8, 26],
      r32: [8, 26],
