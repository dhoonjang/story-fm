...
import { addDays, dayOfWeek, diffDays, sortEntries, squadReturnOf } from "./calendar";
 ...
 * (game-loop.md §2 "주간 훈련 리듬 — 경기일 기준 자동 배치"). 실제 프로 구단의
 ...
 *  - 규칙을 흉내 낸 부분 검사를 쓰지 않는 이유: 예전엔 경기일과 그 다음날만 봐서
 *    MD−1·MD−2·MD+2를 놓쳤고, 마이크로사이클 규칙이 바뀌면 조용히 어긋났다.
 *    설치와 검사가 `planWindow` 하나를 공유하면 그럴 수 없다.
 *
