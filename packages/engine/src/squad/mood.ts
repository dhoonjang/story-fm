...
export type MoodFact =
  ...
  /**
   * **전술 적응도**가 무뎌졌다 (player.md §7.4) — 몸의 예산(`condition`)과 다른
   * 사실이다. 잘 쉬었지만 두 달을 재활실에서, 혹은 클럽 밖에서 보낸 선수가 여기서
   * 갈린다. 등급만 낸다: 감독이 관측하는 것은 출전 기록과 일정이지 숫자가 아니고,
   * 말은 화면·GM이 붙인다.
   */
  | { cause: "familiarity"; tier: FamiliarityTierKey }
  /**
   * **시즌이 몸에 쌓아 둔 것** (player.md §5.5) — 오늘의 체력과도, 전술 적응도와도
...
export function moodFactsOf(
  ...
  if (injury) {
    ...
    /**
     * ── 전술 적응도 ── **몸의 예산과 다른 사실이다.** 잘 쉬어서 체력은 가득한데
     * 두 달을 재활실에서 보내 판이 몸에서 빠진 선수가 있다 (player.md §7.4) — 그
     * 사실을 말하는 카드가 여기다. 「생소」·「서투름」은 언제나 내고(감독이 손을
     * 써야 하는 자리다), 「익히는 중」은 달리 할 말이 없을 때만 낸다 — 신입 기준선
     * (60)도 결장 감쇠의 평형(55)도 그 등급 안이라, 늘 내면 새로 왔거나 한동안
     * 자리를 비운 선수마다 같은 줄이 서서 소음이 된다.
     *
     * ⚠️ **개막 전에는 내지 않는다** — `no-minutes`와 같은 이유이자 같은 문이다.
     * 새 게임의 7월은 선수단 전원이 그 기준선에 함께 서 있어(player.md §7.4),
     * 라커룸 스물다섯 명이 통째로 같은 한 줄이 된다. 남들과 다를 때만 그 선수의
     * 사실이다.
     */
