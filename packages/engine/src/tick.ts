...
import { addDays, dayOfWeek, diffDays, matchesOn, nextMatchFor, windowOpenOn } from "./calendar";
...
import { advanceDomesticCups } from "./domestic-cup";
import { advanceEuroKnockouts } from "./euro-knockout";
import { applyMonthlyDevelopment } from "./development";
import { returnDueLoans, signFreeAgents } from "./departures";
import { decayedForm } from "./form";
import type { TrainedSession } from "./training-report";
import {
  applyAiMatchFinance,
  ensureMonthlyPosted,
  payWeeklyWages,
...
import { runAiTransfers } from "./ai-market";
import { reviewUserSeat, runManagerMarket } from "./manager-market";
import { matchRating } from "./ratings";
import { settleTactics } from "./skills";
import { allMatchesDone, endSeason } from "./season";
import { cancelTrainingOn, syncDefaultTraining } from "./training-plan";
import {
  groupOf,
  activeSuspension,
  assignmentsOf,
 ...
 *
 * 한 경기가 자리에 따라 −51~−75, 회복이 하루 6~14다. **주 1경기 리듬에서는
 * 걸리지 않고**(엿새면 거의 다 찬다) 주중이 낀 사흘 간격에서만 걸린다 — 그게
 * 로테이션이 실제로 필요한 자리다. 62였을 땐 경기 다음 날에나 닿는 값이라
 * 라인업을 짜는 시점엔 언제나 통과했고, AI는 사실상 로테이션을 하지 않았다.
 */
const ROTATION_FATIGUE = 30;
