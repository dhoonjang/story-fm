/**
 * 도착한 스카우트 보고서를 카드로 꺼내는 자리 — **실모드와 mock이 같은 자리를 쓴다.**
 *
 * 한쪽만 꺼내면 다른 쪽에서는 줄만 차오르고 카드는 한 번도 서지 않는다
 * (docs/data/player.md §9.4-1 · docs/llm/agents.md §6). gm.ts와 mock-gm.ts가 서로를
 * 부르는 사이라 규칙이 어느 한쪽에 살면 다른 쪽이 가져다 쓸 수 없어, 둘이 함께
 * 내려다보는 이 모듈이 규칙을 갖는다.
 */
import {
  consumeReportCards,
