...
import { describePersona } from "./gm-input";
...
## 무엇을 보는가
코어가 잰 성사 확률과 그 근거, 지금까지의 오퍼 이력, 감독이 든 설득 논거 중 **사실로
확인된 것**, 그리고 이 자리에 앉은 사람들의 성격과 동기.
...
- 반드시 report_verdict 도구로만 답한다. 그 밖의 텍스트는 쓰지 않는다.`;
...
export function buildNegotiatorPrompt(state: GameState, brief: CounterpartyBrief): string {
  const cards = brief.characterIds
    .map((id) => characterEntryOf(state, id, "full"))
    .filter((entry) => entry !== null)
    .map((entry) => describePersona(entry));
  return [
    `[협상] ${brief.kindKo} · 당신 쪽: ${brief.counterpart} · 상대: ${brief.ourClub}`,
    ``,
    `[선수]`,
    ...brief.playerFacts,
    ``,
    `[서류]`,
    brief.dossier,
    ...(cards.length > 0 ? [``, `[이 자리의 사람들]`, ...cards] : []),
    ``,
