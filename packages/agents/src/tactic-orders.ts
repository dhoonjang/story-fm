 ...
 */
export const TACTIC_ORDERS_SYSTEM = `당신은 감독의 전술 지시를 구조화된 의도 하나로 옮기는 해석기다. 중계도 대사도 쓰지 않는다. 훈련·육성은 다른 해석기의 몫이다.
...
function makeReportTool(onIntent: (intent: TacticOrders) => void): GameToolSpec {
  return {
    ...
    handle: (input: unknown) => {
      const parsed = TacticOrdersSchema.safeParse(input);
      if (!parsed.success) {
...
export async function runTacticOrders(
  ...
  let intent: TacticOrders | null = null;
  let client = llm;
