...
# 판을 바꾸는 명령
- set_lineup — 평시에 선발을 바꾸는 말에. 바뀌는 자리만 싣는다 — 부르지 않은 자리는 지금 선발이 지킨다. 자리는 포지션 코드로, 2군 선수를 올리면 squadLevels에 first로 함께. 벤치에 앉은 선수를 그라운드에 세우는 것도 이 명령이다 — set_player_tactic은 이미 뛰는 선수의 자리만 옮긴다.
