...
export const SKILL_CATALOG = [
  ...
  },
  {
    ...
    readOnly: false,
    description:
      "감독이 열려 있는 기자회견에 답한 결과를 기록한다(스냅샷의 <press>에 기자가 아는 사실이 실려 있다 — 없으면 쓰지 마라). " +
  ...
  },
  {
    ...
    readOnly: false,
    description:
      "세계가 먼저 연 자리에 감독이 답한 결과를 기록한다(스냅샷의 <approach>에 화자와 사실이 실려 있다 — 없으면 쓰지 마라). " +
      ...
      "압력만 되돌린다 — 불만은 talk_to_player·승격·선발로만 풀린다.",
  },
  {
    name: "substitute",
    label: "선수 교체",
  ...
  },
  {
    ...
    description:
      ...
      'view="calendar"는 감독의 달력 — 경기·훈련·이적창 개폐를 한 축에서 날짜순으로 본다. 기본은 오늘부터 14일이고 from·to·days로 범위를, type="training"으로 훈련만 볼 수 있다. 훈련 계획을 묻거나 새 훈련을 잡기 전에 이걸로 확인하라. from이 지난 날이면 그 사이 벌어진 일이 일지로 함께 온다 — 부상·이적·돈·소식(오퍼 답 도착·협상 만료·계약 경고·타 구단 이적·보드 답신).',
  },
  {
    name: "get_career",
    label: "커리어 조회",
  ...
  },
  {
    ...
    readOnly: false,
    description:
      "이적 오퍼를 넣는다. 임대는 fee가 임대료, weeklyWage가 우리(loan)나 그쪽(loan_out)이 낼 주급이다. 넣기 전에 deal_odds로 확률을 확인하라. 답은 며칠 뒤에 오고 같은 조건을 반복하면 상대가 지친다. pitch는 감독이 말로 든 설득 논거를 kind로 옮긴 것이고, 목록에 없는 이야기는 other로 넣으면 서사에만 남는다. 확인된 논거는 확률이 아니라 판정 여유를 연다. 감독이 금액만 말했으면 무엇을 더 걸 수 있는지 먼저 물어라. paymentYears로 이적료를 2~4년 분할로 부를 수 있다 — 예산이 모자랄 때의 길이고, 상대는 늦게 오는 돈을 깎아 보므로 같은 확률을 원하면 총액을 올려 불러야 한다.",
  },
  {
    name: "respond_offer",
    label: "오퍼 판정",
  ...
  },
  {
    ...
    readOnly: false,
    description:
      "합의된 협상을 다음 단계로 넘긴다. 첫 호출은 계약이 아니라 메디컬이다 — 검진 날짜가 잡히고, 통과하면 그날 코어가 이적료·계약·재정·소속을 함께 반영한다. 이 도구를 부른 턴에 도장·입단식·발표·기자회견·등번호를 쓰지 말고, 검진 결과를 기다리는 동안 다시 부르지 않는다. 소견이 나온 뒤 다시 부르면 그것은 소견을 알고도 데려오겠다는 강행이다.",
  },
  {
