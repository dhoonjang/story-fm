...
export const SKILL_CATALOG = [
  ...
  },
  {
    ...
    readOnly: false,
    description:
      "오퍼에 답한다 (accept·counter·reject) — **누가 답할 차례인지는 협상이 안다.** 우리가 넣은 오퍼면 당신이 상대 구단·선수·에이전트가 되어 판정하고, 상대가 넣은 오퍼면 감독의 뜻대로 답한다. 답이 도착한 협상은 서사만 쓰지 말고 반드시 이 도구로 기록해야 다음 단계로 간다. deal_odds의 확률과 근거를 앵커로 삼고 note에 한 줄을 남긴다. **설득 논거가 붙은 협상은 확률만 보고 거절하지 마라** — 그 이야기가 이 선수에게 얼마나 큰지는 그 선수의 나이·처지·이력을 보고 당신이 판정한다. counter는 방향이 갈린다: 영입이면 더 부르고(우리 제시액 이상·요구액 +15% 이하), 매각이면 사는 쪽이 깎아 부르고(시장가 55% 이상·우리 호가 미만), 재계약이면 선수가 주급을 부른다(우리 제시액 초과·기대치 1.4배 이하), 해지면 선수가 정산금을 올려 부른다(우리 제시액 초과·일방 해지 전액 이하).",
  },
  {
    name: "accept_deal",
    label: "계약 확정",
