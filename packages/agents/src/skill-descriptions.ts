...
export const SKILL_CATALOG = [
  ...
  },
  {
    ...
    readOnly: false,
    description:
      "**상대가 넣은 오퍼**에 감독의 뜻대로 답한다 (accept·counter·reject). 우리가 넣은 오퍼에 대한 상대의 답은 당신이 판정하지 않는다 — 그 자리는 이미 끝나 상태 스냅샷의 📨 줄에 있고, 당신은 그것을 장면으로 전한다. 감독이 답을 정한 협상은 서사만 쓰지 말고 반드시 이 도구로 기록해야 다음 단계로 간다. deal_odds의 확률과 근거를 앵커로 삼고 note에 한 줄을 남긴다. counter는 우리가 값을 올려 부르는 것이고(받은 값 초과), **paymentYears**를 얹으면 같은 금액을 분할로 되부르는 것이다 — 총액 한도는 그대로 걸리고 분할은 그 위의 흥정이다.",
  },
  {
    name: "accept_deal",
    label: "계약 확정",
