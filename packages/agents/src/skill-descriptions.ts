...
export const SKILL_CATALOG = [
  ...
  },
  {
    ...
    readOnly: false,
    description:
      "무직인 감독이 최근 공석(경질 뒤 14일 안)인 구단에 먼저 지원한다 — 지원할 수 있는 공석은 상태 스냅샷에 선다. " +
      "평판이 그 등급의 문턱을 넘으면 그 자리에서 제안이 서고(먼저 아쉬운 쪽이라 연봉은 기본보다 짜다), 못 미치면 거절당한다. " +
      "열린 제안이 있는 동안에는 지원할 수 없다 — 답할 자리는 한 번에 하나다.",
  },
  {
    name: "get_finance",
    label: "재정 조회",
  ...
  },
  {
    ...
    readOnly: true,
    description:
      "이 조건이면 이적이 성사될지 코어가 계산한 확률과 그 근거(요구액·주급 기대치·기여 항목)를 준다. 감독에게 답하기 전에 반드시 확인하고, 그 근거는 사람의 말로 풀어 전하라. 확률이 낮다고 포기하지 마라.",
  },
  {
  ...
  },
  {
    ...
    readOnly: false,
    description:
      "이적 오퍼를 넣는다. 임대는 fee가 임대료, weeklyWage가 우리(loan)나 그쪽(loan_out)이 낼 주급이다. 넣기 전에 deal_odds로 확률을 확인하라. 답은 며칠 뒤에 오고 같은 조건을 반복하면 상대가 지친다. **pitch**는 감독이 말로 든 설득 논거를 kind로 옮긴 것이고, 목록에 없는 이야기는 other로 넣으면 서사에만 남는다. 확인된 논거는 확률이 아니라 판정 여유를 연다. 감독이 금액만 말했으면 무엇을 더 걸 수 있는지 먼저 물어라. **paymentYears**로 이적료를 2~4년 분할로 부를 수 있다 — 예산이 모자랄 때의 길이고, 상대는 늦게 오는 돈을 깎아 보므로 같은 확률을 원하면 총액을 올려 불러야 한다.",
  },
  {
    name: "respond_offer",
    label: "오퍼 판정",
    ...
    readOnly: false,
    description:
      "오퍼에 답한다 (accept·counter·reject) — **누가 답할 차례인지는 협상이 안다.** 우리가 넣은 오퍼면 당신이 상대 구단·선수·에이전트가 되어 판정하고, 상대가 넣은 오퍼면 감독의 뜻대로 답한다. 답이 도착한 협상은 서사만 쓰지 말고 반드시 이 도구로 기록해야 다음 단계로 간다. deal_odds의 확률과 근거를 앵커로 삼고 note에 한 줄을 남긴다. **설득 논거가 붙은 협상은 확률만 보고 거절하지 마라** — 그 이야기가 이 선수에게 얼마나 큰지는 그 선수의 나이·처지·이력을 보고 당신이 판정한다. counter는 방향이 갈린다: 영입이면 더 부르고(우리 제시액 이상·요구액 +15% 이하), 매각이면 사는 쪽이 깎아 부르고(시장가 55% 이상·우리 호가 미만), 재계약이면 선수가 주급을 부른다(우리 제시액 초과·기대치 1.4배 이하), 해지면 선수가 정산금을 올려 부른다(우리 제시액 초과·일방 해지 전액 이하). **paymentYears**를 얹으면 같은 금액을 분할로 되부르는 것이다 — 총액 한도는 그대로 걸리고 분할은 그 위의 흥정이다.",
  },
  {
    name: "accept_deal",
    label: "계약 확정",
  ...
  },
  {
    ...
    readOnly: false,
    description:
      "우리 선수와의 계약을 **일방 해지**한다 (방출). 잔여 계약 주급 **전액**을 즉시 물고 원장에 남아 PSR에 잡힌다. " +
  ...
  },
  {
    ...
    readOnly: false,
    description:
      "우리 선수에게 **합의 해지**를 제안한다 — severance는 제시 정산금이고 상대는 선수 본인이다. 이적창과 무관하다. " +
