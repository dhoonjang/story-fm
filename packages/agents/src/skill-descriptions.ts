...
export const SKILL_CATALOG = [
  ...
  },
  {
    ...
    readOnly: true,
    description:
      "이 조건이면 이적이 성사될지 코어가 계산한 확률과 그 근거(요구액·주급 기대치·기여 항목)를 준다. 상태를 바꾸지 않으므로 감독에게 답하기 전에 반드시 확인하고, 그 근거는 사람의 말로 풀어 전하라. kind를 sell로 주면 매각 방향(사는 쪽이 그 값을 낼지·선수가 떠날지)으로 계산한다. pitch를 함께 주면 **설득 논거를 미리 시험**할 수 있다 — 어떤 이야기가 사실로 통하고 어떤 게 거짓으로 드러나는지 근거에 한 줄씩 나온다. 확률이 낮다고 포기하지 마라: 돈으로 안 되는 딜을 논거로 넘는 것이 이 게임의 이적이다.",
  },
  {
