...
export const SKILL_CATALOG = [
  ...
  },
  {
    ...
    description:
      ...
      "인자가 없다 — 이번 턴 감독의 말은 코어가 해석기에 넘긴다. 한 턴에 한 번 부른다. 결과로 무엇이 걸렸고 무엇이 반려됐는지가 온다. 반려된 대로 쓴다. " +
      "옮기지 못한 지시가 돌아오면 그 말이 걸리지 않았음을 이 턴의 장면에서 감독에게 말하고 무엇을 정해야 하는지 되묻는다 — 걸린 줄 알고 다음 경기를 맞는 것이 가장 나쁘다. " +
      '감독이 정하지 않고 맡긴 말("알아서 짜세요")에는 부르지 않는다 — 코치의 안을 장면으로 내놓고 감독이 못 박은 턴에 부른다. ' +
      "훈련·육성은 training_orders, 이적·재정은 market_orders다 — 한 말에 섞여 있으면 각각 부른다. 회견·대화는 각자의 도구가 있다.",
  },
  {
    ...
    description:
      ...
      "인자가 없다 — 이번 턴 감독의 말은 코어가 해석기에 넘긴다. 한 턴에 한 번 부른다. 결과로 무엇이 걸렸고 무엇이 반려됐는지가 온다. " +
  ...
  },
  {
    ...
    description:
      ...
      "outcome은 맥락 적합성·설득 근거·수용성으로 판정하고, intensity는 발화 강도에 맞춘다(1 담담히 ~ 3 격하게). " +
      "inspired는 드문 판정이다 — 대화가 그 사람의 처지에 정확히 닿고 근거가 섰을 때만이고, 평범한 격려는 encouraged다. " +
  ...
  },
  {
    ...
    description:
      ...
      "구단주가 건 조건을 두고 감독이 되물었으면 counter에 싣는다 — 기한을 늘려 달라면 extendDays, 조건을 낮춰 달라면 relax: true. 한 차례뿐이다. " +
      "압력이 연 자리는 압력만 되돌린다 — 불만은 이름을 불러 건넨 대화·승격·선발로만 풀린다. " +
      "이적 요청이 선 선수의 자리에서 답하면 요청도 닫힌다 — 팔지·거부할지만 market_orders로. " +
  ...
  },
  {
    ...
    description:
      ...
      "인자가 없다 — 이번 턴 감독의 말은 코어가 해석기에 넘긴다. 한 턴에 한 번 부른다. 조건(추가 영입·주장·등번호·바이아웃 조항·사이닝 보너스·주급 인상 조항·그 밖)도 오퍼·재계약에 실린다. 오퍼 전에는 deal_odds로 확률을 보고 감독과 값을 정한 턴에 부른다. 결과로 무엇이 걸렸고 무엇이 반려됐는지가 온다. " +
      "우리 오퍼에 온 상대의 답은 여기가 아니다 — <letters>가 나른다. 마주 앉은 자리의 말은 speak_at_table이다.",
  },
  {
    name: "speak_at_table",
    label: "테이블",
    ...
    readOnly: false,
    description:
      "협상 상대와 마주 앉아 감독의 말을 건넨다 — line은 감독의 말 원문이다. 협상이 없으면 playerId와 kind로 자리를 연다(오퍼 없이 — 이레 안에 오퍼가 없으면 닫힌다). 테이블에서 감독이 말한 값·조건·답은 이 도구가 옮긴다 — market_orders를 함께 부르지 않는다. " +
      "상대의 답이 <reply speaker name>으로 돌아오고, 그 답은 그 사람의 말로 장면에 옮기되 지어내지 않는다. 영입의 테이블에는 값을 답하는 구단과 개인 조건을 답하는 선수 쪽이 함께 앉아 답이 둘일 수 있다 — 누구에게 건네는 말인지는 line의 내용이 정한다. 오퍼가 올라 있으면 상대는 그 자리에서 답한다. 상대가 조건을 부르면 [장부] 줄에 서고 감독이 들어주거나 거절한다. " +
      "감독이 든 논거는 상대가 듣고 코어가 사실을 가린다 — 당신이 pitch로 옮기지 않는다. 인내가 0이 되면 상대가 일어나고 협상은 이번 창에서 끝난다.",
  },
