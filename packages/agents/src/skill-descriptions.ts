...
export const SKILL_CATALOG = [
  ...
  },
  {
    ...
    description:
      ...
      "**playerId에 선수 이름을 적으면 그 한 명의 상세 카드**를 돌려준다 — 능력치·컨디션·계약·배치에 더해 부상 이력(횟수·누적 결장일·부위), 이번 시즌 경고 누적과 정지까지 남은 장수, 이동 이력까지. " +
      '"부상이 잦은가", "다음 경기 경고 조심해야 하나" 같은 판단은 그 이력으로 하라. 컨텍스트에는 선수단 이름밖에 없으므로, 감독이 특정 선수를 두고 물으면 그 선수를 논하기 전에 먼저 호출한다.',
  },
  {
    name: "get_squad",
    label: "스쿼드·라인업 조회",
    ...
    description:
      ...
      "라인업·포지션·교체를 논하기 전에 반드시 호출하라 — 컨텍스트에는 선수단 이름뿐이라 지금 누가 어디에 서 있는지는 여기서만 알 수 있다. " +
  ...
  },
  {
    ...
    readOnly: true,
    description:
      "이 조건이면 이적이 성사될지 코어가 계산한 확률과 그 근거(요구액·주급 기대치·기여 항목)를 준다. 상태를 바꾸지 않으므로 감독에게 답하기 전에 반드시 확인하고, 그 근거는 사람의 말로 풀어 전하라. kind는 다섯 갈래다 — buy(기본)·sell·renew·loan·loan_out. sell·loan_out은 teamId를 함께 줘야 **그 구단 협회의 이적창**으로 판정한다. pitch를 함께 주면 **설득 논거를 미리 시험**할 수 있다 — 어떤 이야기가 사실로 통하고 어떤 게 거짓으로 드러나는지 근거에 한 줄씩 나온다. 확률이 낮다고 포기하지 마라.",
  },
  {
