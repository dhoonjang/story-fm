...
export const SKILL_CATALOG = [
  ...
  },
  {
    ...
    readOnly: false,
    description:
      "감독이 협상 상대와 마주 앉으려 할 때 — 상대 구단의 단장이나 선수의 에이전트와 직접 이야기하겠다고 하거나, 어느 선수와 협상을 열자고 할 때 부른다. " +
      "한 방에는 한 사람이 앉는다: party=club은 단장(이적료·분할·기한), party=agent는 에이전트(주급·연수·지위·조건). 영입·임대는 감독이 누구를 만나겠다고 했는지로 고르고, 말하지 않았으면 비운다(구단 쪽이 먼저다). 재계약은 에이전트뿐이다. " +
      "협상 방이 열리고 감독에게 자리에 앉는 확인 창이 뜨므로 이 턴에는 그 자리로 향하는 장면까지만 쓰고 상대의 말은 쓰지 않는다. 감독이 앉으면 협상 마스터가 진행한다. " +
      "열린 협상이 있으면 list_negotiations의 negotiationId로, 없으면 playerId와 kind(buy·renew·loan)로 오퍼 없는 자리를 연다 — 이레 안에 오퍼가 없으면 닫힌다. " +
      "상대 없이 오퍼를 넣고 접고 답하는 것은 market_orders다.",
  },
  {
    name: "tactic_orders",
    label: "전술 지시",
  ...
  },
  {
    ...
    description:
      ...
      "협상을 담당자(수석코치·코치·스카우트)에게 한도를 주고 맡기는 말과 걷는 말도 여기다 — 맡긴 협상은 감독 턴 없이 굴러 <mandates>가 그 진행을 나른다. " +
      "우리 오퍼에 온 상대의 답은 여기가 아니다 — <letters>가 나른다. 마주 앉는 것은 start_negotiation이다.",
  },
