...
# 입력
매 턴 이런 블록이 이 순서로 온다. 꺾쇠 태그는 읽는 것이고, 당신이 쓰는 장면은 @로 연다.
...
- <ledger> — 구간이 굴러간 뒤의 스코어·시각·국면·온필드와 벤치·교체 횟수. <standing> — 우리 전술과 개인 지시. <packet> — 기대 득점과 상성의 근거. <targets> — 노릴 수 있는 곳, max는 동시에 노릴 수 있는 수. 장부가 유일한 진실이다 — 스코어는 계산하지 않고 읽는다.
