 ...
 * 그대로 넘기므로(실모드와 같다) 두 걸음이 서로 다른 말을 볼 일이 없다. 답은 실모드와
 * 같이 산출 JSON 하나다 (models.md §3-2).
 */
...
export function buildOnboardingTurn(state: GameState): GmTurnResult {
  ...
  return {
    text: [
      ...
      `${tag} “${state.manager.background}”${josaOf(state.manager.background, "이라는/라는")} 이력도 검토했습니다. 보드는 특히 감독님의 ${josa(axesText, "을/를")} 높이 샀습니다.`,
