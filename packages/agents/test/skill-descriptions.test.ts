...
describe("스킬 설명 — 코드가 유일한 원본이다", () => {
  ...
  it("도구 수·그룹별 수·읽기 전용 수가 문서의 표와 같다", () => {
    ...
    expect(SKILL_CATALOG.length).toBe(51);
    expect(SKILL_CATALOG.filter((s) => s.readOnly).length).toBe(10);
  });
});
...
describe("규칙이 사는 자리", () => {
   ...
   * 두 줄이 붙어도 아무 데서도 드러나지 않는다. 상한은 지금 총량(≈11,167자)에 한 도구
  ...
  it("설명은 길이 예산 안에 있다", () => {
    ...
    expect(total).toBeLessThanOrEqual(11_570);
  });
