...
- **LLM**: 역할별 멀티 프로바이더 — 에이전트마다 제공자와 모델을 독립 설정한다.
  현재 GM·경기 중계=Gemini 3.6 Flash, 결산 3종=Gemini 3.5 Flash Lite.
  설정과 모델 ID는 `config/llm.yml`에서 단일 관리
  (→ [docs/design/llm.md](./docs/design/llm.md))
...
1. **모델 ID 하드코딩 금지** — `config/llm.yml`에서 에이전트별 제공자·모델을
   단일 관리한다. 프로바이더 차이는 `packages/llm` 어댑터가 흡수한다 (→ llm.md).
...
| LLM 에이전트 설정·입출력·스킬 표면 | [llm.md](./docs/design/llm.md) |
