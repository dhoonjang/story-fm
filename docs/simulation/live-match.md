...
형태는 `shape.ts`, 역할의 활동 범위는 `heatmap.ts`, 역할 성향은 `roles.ts`, 전술
파라미터는 `params.ts`, 이동과 공의 비행은 `physics.ts`, xG는 `xg.ts`, 한 틱의 판단·경합·
재시작·골키퍼는 `step.ts`, 하프와 교체·벤치를 잇는 러너는 `runner.ts`다. 이 식들이 읽는 **숫자는 전부 `tuning.ts`** 한 파일에
...
- **라인의 바닥** — 수비 형태는 페널티 스폿 근처보다 깊이 서지 않는다. 공이 그보다 골라인
  가까이 가면 공을 따라서만 내려간다. 센터백·풀백은 이 라인에서 뒤로 처질수록 가치장(§5.2)의
  벌점이 커져, 골문 앞에 모여 서지 않는다.
- **라인 올리기** — 공 가진 상대가 우리 골 반대쪽으로 움직이는 만큼(연속으로) 수비 라인의
  기준이 올라서서 오프사이드 선을 세운다.

### 3.3 히트맵 — 자리는 점이 아니라 분포다 (`live/heatmap.ts`)

형태 자리는 말이 **기댈 중심**이고, 말이 실제로 다니는 곳은 그 둘레의 **히트맵**이다.
히트맵은 경기장 위의 연속 분포로, 말의 선택을 확률로 기울인다(§5.2). 안과 밖을 가르는
경계는 없다 — 밀도는 어디서나 0보다 크고 중심에서 멀어질수록 매끄럽게 준다.

```
히트맵 = 성분 몇 개의 가중합
ρ(점) = Σᵢ wᵢ · g(점; 성분ᵢ)
g     = exp(−½·(u_d² + u_l²))
        u_d = 깊이 차 ÷ (뒤쪽이면 σ_back, 앞쪽이면 σ_ahead),  u_l = 가로 차 ÷ σ_side
```

- **성분**은 한 역할이 한 국면에 머무는 **자리 하나**다 — 중심 오프셋 · 뒤/앞/옆 퍼짐 ·
  가중치. 앞과 뒤의 퍼짐이 따로라 한쪽으로 긴 분포가 선다. `exp`는 `dmath`의 결정적
  구현이다(§8.2).
- **중심은 전술판과 블록이 옮긴다.** 성분의 중심은 형태 자리(§3.2)에서의 오프셋이다.
  전술판에서 선수를 옮기면 분포 전체가 함께 옮겨 가고, 블록이 공을 따라 오르내리고
  쏠리면 분포도 따라간다.
- **무게가 옮는다.** 성분의 가중치는 공의 깊이 · 국면 · 멘탈리티에 연속으로 반응한다.
  풀백의 주 성분은 **측면 길게** — 제 측면을 따라 자기 수비 지역부터 상대 진영까지 앞뒤로
  길고 옆으로 좁다. 여기에 제자리 성분(수비 쪽)과 측면 끝 성분(상대 마무리 지역의 골라인
  근처 — 크로스를 올리는 자리)이 얹히고, 공이 전진할수록 앞쪽으로 무게가 옮되 제자리 성분이
  다 꺼지지는 않는다. 측면 끝 성분은 `advance`가 크게 기울여 윙백에게 특히 무겁다.
  가담과 복귀는 따로 된 분기가 아니라 한 분포에서 나온다. 윙어는 폭 성분과 안쪽 성분을,
  최전방은 라인 앞 성분과 박스 성분을 갖는다.
- **역할이 성분을 기울인다.** 성향(§4)이 가중치와 퍼짐을 연속으로 옮긴다 —
  `advance`·`runBehind`·`boxPresence`는 앞쪽 성분의 무게와 앞 퍼짐을, `cover`는 뒤쪽을,
  `roam`·`inside`는 옆 퍼짐과 안쪽 성분을. 윙백은 가담 성분이 무겁고, 노-넌센스 풀백은
  가볍다.
- **표 하나가 모양을 쥔다.** 자리 묶음 × 국면마다의 성분(중심 오프셋 · 퍼짐 · 기본
  가중치 · 가중치가 반응하는 입력)은 `tuning.ts`의 한 표다. 모든 퍼짐에 곱하는 배율 하나가
  모양은 두고 범위만 넓히고 좁힌다. 성분의 중심은 늘 경기장 안에 선다.
- **세트피스 성분** — 킥 직후의 세트피스 국면에는 그 말이 섰던 배치 자리(§5.5)를 중심으로
  한 좁은 성분이 얹힌다. 가중치는 킥 순간이 가장 크고 창이 끝날 때까지 연속으로 줄어
  평소 국면의 성분에 무게를 넘긴다. 공이 멈춘 동안의 배치 자체는 히트맵이 아니라 재시작
  배치가 정하고, 골키퍼의 자리(§5.6)는 기하가 정하는 한 점이라 히트맵을 쓰지 않는다.
...
성향은 두 곳에 닿는다 — **히트맵의 성분**(§3.3, 말이 어디를 얼마나 다니는가)과 **가치장의 항**
(§5.2 — 전진 성향이 위협의 무게를, `cover`가 마크의 무게를 정한다).
...
| 선택지    | 성공을 가르는 것                                                                                                   |
| --------- | ------------------------------------------------------------------------------------------------------------------ |
| 슈팅      | 슈팅 xG(§5.4) — 거리·각도·압박·길목의 수비·발/머리. 전술의 문턱 xG를 넘어야 선택지에 서고, 문턱은 박스 안에서 낮다 |
...
### 5.2 공이 없는 말 — 가치장 (`live/step.ts`)

공 없는 말은 판단 시각마다 경기장 위의 **모든 점 x에 가치 U(x)**를 매기고, 그 분포에서
목표를 **뽑는다**. 후보를 켜고 끄는 분기는 없다 — 압박·마크·추격·지원·침투는 따로 고르는
행동이 아니라 U의 항이고, 모든 항은 경계 없는 연속 함수다. 거리를 쓰는 곳은 끊는 선 대신
옅어지는 폭(σ)을 쓴다.

```
U(x) = β · ln ρ(x)                                   역할의 분포 (히트맵, §3.3)
     + a · [ 패스 받기(x) + 빈 공간(x) + 위협(x) − 오프사이드 위험(x) ]
     + (1 − a) · [ 압박(x) + 마크(x) + 골 쪽 커버(x) − 라인 이탈(x) ]
     + 추격(x) + 지시(x) − 붐빔(x) − 이동 비용(x) + 관성(x)

P(x) ∝ exp(U(x) ÷ T)          T = 능력치(공격 중 오프더볼 · 수비 중 위치선정)
```

- **공격 무게 a** — 공을 가진 편 1, 아닌 편 0. 소유가 바뀐 직후에는 반대쪽 몫이 남았다가
  몇 초에 걸쳐 연속으로 사라진다(전환).
- **패스 받기** — 공 가진 동료에서 x까지의 경로에 상대가 가까울수록 곱으로 줄고(열린 길),
  전술의 패스 길이에서 가장 크다. **빈 공간**은 가장 가까운 상대까지의 거리가 연속으로
  채운다. **위협**은 xT를 칸 사이로 보간한 값에 역할의 전진 성향과 멘탈리티가 곱해진다.
  **오프사이드 위험**은 상대 라인(과 공)을 넘는 정도의 시그모이드다.
- **압박** — 편마다 틱에 한 번, 압박 자리(공 가진 상대와 우리 골문 사이)에 닿는 시간의
  softmax로 **압박 몫**을 나눈다. 몫의 합은 압박 강도 — 첫 수비수 1에 압박 인원 × 공이
  압박선 안에 든 정도, 역압박, 공의 위험이 더해진다. 공보다 상대 쪽에 선 말은 늦게 닿는
  것으로 친다(등 뒤에서는 길을 막지 못한다).
- **마크** — 상대마다 수비들의 비용(거리 − 제 히트맵에서 그 상대 자리의 로그 밀도)의
  softmax로 **마크 몫**을 나눈다. 그 상대의 골 쪽 자리 둘레에 몫 × 위협만큼 가치가 선다.
  뒤를 지키는 성향(`cover`)이 낮은 역할(최전방)은 마크의 가치가 작다.
- **골 쪽 커버** — 공과 우리 골문을 잇는 선분 둘레에 공의 위험(우리 골문까지의 거리가
  옅게 한다)만큼 가치가 서고, 동료가 이미 선 자리는 값이 준다.
- **라인 이탈** — 센터백·풀백은 수비 라인(형태 자리의 깊이, 공 가진 상대가 뒤로 몰면 그만큼
  올라선다)에서 벗어난 만큼 벌점을 받는다. 뒤로 처지는 쪽이 더 무겁다.
- **추격** — 주인 없는 공, 또는 상대 패스가 떨어질 자리에 닿는 시간의 softmax로 **추격 몫**을
  나눈다. 상대가 먼저 닿을 공은 덜 쫓는다. 우리 패스는 받을 말이 간다.
- **지시** — 판독기의 개인 지시(§6.2)가 가리키는 자리 둘레의 큰 가치.
- **붐빔 · 이동 비용 · 관성** — 동료 둘레는 값이 줄고, 먼 점은 거리만큼 덜 값지며, 지금
  목표 둘레는 값이 조금 높다(판단마다 목표가 뒤집히지 않는다).
- **뽑는 법** — 경기장 전체를 매번 매기지 않는다. 제안점을 히트맵과 기준점(지금 위치·목표·
  공·공 가진 동료·압박 자리·마크 자리·골 쪽 선·상대 라인) 둘레에서 뽑고, 제안 밀도로 나눈
  가중치로 하나를 고른다 — 같은 분포를 싸게 뽑는 방법일 뿐 선택을 바꾸지 않는다.
- **행동 이름** — 뽑힌 점에서 가장 크게 기여한 항이 행동(압박·마크·커버·추격·지원·침투)이
  되고, 이동의 급함(§5.3)이 그것을 읽는다.
- **급함도 연속이다** — 되돌아갈 거리, 공의 위험, 전환 직후의 정도가 시그모이드로 급함을
  올린다. 압박선 밖·등 뒤의 압박은 따라붙기만 한다.

골키퍼는 가치장을 쓰지 않는다(§5.6).
...
- **공의 비행**: 패스·슈팅·크로스는 속도와 높이를 가진 비행이다. 오차는 킥 순간에
  ...
  패스 능력의 몫은 지수다 — 리그 평균의 패서에서 기준이고 15점마다 오차가 절반·두 배가 되어,
  실측 팀 패스 성공률의 팀 간 퍼짐(sd 4.9%p)이 패서의 능력 차에서 나온다.
...
- **차단**: 비행 중인 공의 경로에 상대 말이 먼저 닿으면 차단이다. 누가 먼저 닿는가는
  각 말의 위치·속도·반응으로 계산하고, 패스를 끊는 반경은 수비의 읽기(위치선정·태클)가
  정한다 — 리그 평균의 수비가 기본 반경에 서고, 팀 간 수비 수준의 차이가 공을 끊는 빈도로
  선다. 패스를 고르는 말도 같은 반경으로 길목을 읽고, 정확한 패서는 그 곁을 좁게 꿴다.
...
- **키커의 걸음**: 키커는 재시작을 기다리는 동안 다른 말처럼 자리 잡는 걸음으로 공 뒤
  1m(제 골 쪽)까지 가서 선다 — 차러 가는 길은 전력질주가 아니라 체력을 거의 쓰지 않는다. 킥
  1초 전에 그 자리에서 공으로 도움닫기를 하고 찬다.
...
- **클라이언트가 시계를 소유한다.** 말의 이동·판단·경합·장부·AI 벤치까지 전부
  `packages/sim`의 순수 함수로 브라우저에서 굴린다. 감독이 입력하거나 턴이 도는 동안에는
  x1로 흐른다 — 서버에 묻지 않는다(§8.3).
...
- 전술 편집·저장 실패·연결 오류는 정지 사유다. 모든 사유가 해제되어야 재개한다.
- **말하는 동안은 x1로 흐른다.** 감독이 입력하는 동안과 턴(감독의 지시 턴 · 정지점 턴)이
  도는 동안 경기는 서지 않고 실제 시간 속도(x1)로 이어진다. 그 밖에는 기본 배속(x8)이다.
- 경기의 중단(골·파울·스로인)은 코어가 재개한다. 하프타임·연장 시작은 감독이 재개한다.
- 배속은 벽시계와 경기 시계의 비율이다. 계산 간격·난수·판정 규칙을 바꾸지 않는다.
  탭이 숨겨지면 멈춘다.
- **턴 동안 굴린 구간은 확정하지 않는다.** 턴은 서버의 확정 tick 위에서 돌고 판독·지시도
  그 tick에 앉는다. 실행기는 턴 동안 x1로 앞서 굴리되 체크포인트를 내지 않고, 턴이 끝나
  확정 상태를 이어받을 때 **앞서 간 만큼을 그 상태에서 다시 굴린다** — 지시가 반영된 경기가
  보던 자리에서 이어진다. 앞서 굴리는 중에 정지점이 나거나 체크포인트 간격을 채우면 그
  자리에서 선다.
...
- **장부의 사실은 전부 보인다.** 스코어, 사건, 선수·팀 통계(점유·슈팅·xG·패스·태클·
  뛴 거리), 교체, 상대 벤치의 전환.
- **판독은 포인트 문장까지만 보인다.** 감독의 분석 능력이 허락하는 상위 줄(`POINTS_SEEN`)이
  매치 GM에게 가고, 같은 줄이 화면의 「경기 기록」 탭에 선다. 시트는 화면에 서지 않는다.
...
| 하네스             | 무엇을 재는가                                                                                                                                                                                                                   |
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `live-match-stats` | 실제 팀들로 굴린 경기의 팀 통계(득점 분포·슈팅·유효슈팅·xG·패스·성공률·점유·태클·인터셉트·드리블·파울·카드·코너·크로스·오프사이드·거리·경기 길이·볼 인플레이)의 평균·중간값·sd, 슈팅 순간 골 쪽 수비 수, 풀백의 경기 중 깊이 폭 |
