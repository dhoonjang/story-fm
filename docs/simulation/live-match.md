...
### 3.3 히트맵 — 자리는 점이 아니라 범위다 (`live/heatmap.ts`)

형태 자리는 말이 **기댈 중심**이고, 말이 실제로 다니는 곳은 그 중심을 둘러싼 **히트맵**
이다. 히트맵은 매 판단의 국면(공격/수비)에서 다시 선다.

```
히트맵 = { 중심(형태 자리), 뒤 퍼짐, 앞 퍼짐, 옆 퍼짐 }
밀도(점) = k(깊이 차 ÷ 앞/뒤 퍼짐) × k(가로 차 ÷ 옆 퍼짐),  k(u) = (1 − u²)²  (|u| < 1), 아니면 0
```

- **범위가 있다.** 밀도는 퍼짐 안에서만 서고 밖은 0이다 — 센터백은 상대 박스에 서지
  않고, 최전방은 자기 박스로 내려오지 않는다. 앞과 뒤의 퍼짐이 따로라 한쪽으로 긴
  범위가 선다.
- **중심은 전술판과 블록이 옮긴다.** 전술판에서 선수를 옮기면 히트맵의 중심이 함께
  옮겨 간다. 블록이 공을 따라 오르내리고 쏠리면 중심도 따라간다.
- **퍼짐은 자리 묶음이 정하고 역할이 넓힌다.** 공격 국면의 앞 퍼짐은 `advance`·
  `runBehind`·`boxPresence`가, 옆 퍼짐은 `roam`·`inside`가, 수비 국면의 뒤 퍼짐은
  `cover`가 늘린다. 멘탈리티(`commit`)가 공격 국면의 앞 퍼짐에 곱해진다.
- **풀백·윙백은 자기 측면을 통째로 덮는다.** 공격 국면에서 중심이 공을 따라 크게
  오르고 앞 퍼짐이 상대 마무리 지역까지 닿는다. 수비 국면에서는 중심이 수비 라인에
  서고 뒤 퍼짐이 자기 박스까지 닿는다. 가담과 복귀는 따로 된 분기가 아니라, 공의 위치와
  국면에 따라 이 무게가 앞뒤로 옮는 한 모델에서 나온다.
...
성향은 두 곳에 닿는다 — **히트맵의 퍼짐**(§3.3, 말이 어디까지 다니는가)과 **후보점과
...
공 없는 말은 **후보점**을 몇 개 세우고, 점마다 **히트맵 밀도 × 상황 가치**를 매겨 가장
큰 곳으로 간다. 밀도가 0인 점(범위 밖)은 가치가 아무리 커도 고르지 않는다 — 역할이
다니지 않는 곳에는 가지 않는다. 가치에는 판단의 잡음이 섞인다.

```
점수(후보) = 밀도(히트맵, 후보) × (상황 가치(후보) + 역할 가산(후보 종류))
```
...
침투는 라인이 높을수록(뒤 공간이 넓을수록), 침투 성향이 클수록 잦다. 멘탈리티는 앞
퍼짐을 늘려 더 많은 말의 범위에 침투·박스 진입 점이 들어오게 한다. 오버래핑을 고른
풀백은 그 달리기를 잠시 지킨다.
...
- **마크는 존에서 배정된다.** 매 틱 위협이 큰 상대부터, 그 상대가 자기 히트맵 안에 선
  수비 가운데 가장 가까운 말에게 한 명씩 붙는다. 상대가 그 범위를 벗어나면 마크는 다음
  범위의 말에게 넘어간다 — 따라 뛰지 않고 넘긴다. 최전방(CF·ST)은 사람을 잡지 않는다.
- **골 쪽 인원** — 공이 우리 진영에 깊을수록 수비 히트맵의 뒤 퍼짐이 박스 쪽으로 늘고,
...
- **침투 추적** — 마크한 상대가 멀어지면 범위 안에서는 따라 뛴다. 범위 밖으로 나가면
  넘긴다(위).
