...
- **라인의 바닥** — 수비 라인은 페널티 스폿 근처보다 깊이 내려서지 않는다. 공이 그보다 골라인
  가까이 가면 공을 따라서만 내려간다. 센터백·풀백은 어느 후보점을 골라도 이 바닥 밑으로
  서지 않고, 우리 골 쪽으로 파고드는 상대를 따라가는 마크만 예외다. 골문 앞에 모여 서면 박스
  근처의 공 가진 말을 막지 못하고 오프사이드 선도 사라진다.
- **라인 올리기** — 공 가진 상대가 우리 골 반대쪽으로 움직이면(돌아서거나 뒤로 몰면) 센터백·
  풀백이 함께 몇 걸음 올라서서 오프사이드 선을 세운다.
...
성향은 두 곳에 닿는다 — **히트맵의 성분**(§3.3, 말이 어디를 얼마나 다니는가)과 **후보점과
선택지의 가치**(§5, 그 범위 안에서 무엇을 고르는가).
...
공 없는 말은 판단 시각마다 **후보점**을 세우고, 점수에 비례한 확률로 하나를 **뽑는다**
(softmax). 가장 큰 점을 고르지 않는다 — 같은 상황에서도 말은 여러 곳으로 가고, 그 빈도가
히트맵과 상황의 가치를 따른다.

```
점수ᵢ = 상황 가치ᵢ + 역할 가산ᵢ + β · ln ρ(점ᵢ) + 관성ᵢ
P(i)  = exp(점수ᵢ / T) ÷ Σⱼ exp(점수ⱼ / T)
```

- **히트맵은 확률을 기울인다.** 밀도는 `ρ^(β/T)`의 배율로 선택 확률에 들어간다. 분포의
  바깥 점도 가치가 아주 크면(텅 빈 뒷공간) 낮은 확률로 고른다 — 금지가 아니라 드묾이다.
- **온도는 능력치다.** 공격 중에는 `offTheBall`, 수비 중에는 `positioning`이 높을수록
  `T`가 낮다 — 좋은 선수는 가치가 큰 점을 더 일관되게 고르고, 나쁜 선수는 흩어진다.
- **분포에서 뽑은 점** — 후보에는 상황이 만든 점 외에 히트맵에서 뽑은 점 하나가 늘 들어간다.
  표본은 말마다 어긋난 몇십 초의 창마다 한 번 선다 — 창 안에서는 분포가 움직일 때만 따라
  움직인다. 할 일이 없을 때도 말은 제 분포 안을 다닌다.
- **관성** — 지금 가는 목표에 가까운 후보와, 지난 판단과 같은 행동의 후보에 가산을 준다.
  상황이 바뀌어야 다른 일로 옮긴다 — 판단마다 목표가 뒤집히면 말이 쉬지 않고 뛴다.
  오버래핑을 고른 풀백은 그 달리기를 잠시 지킨다.
...
| 자리          | 형태 자리                                | 기본값                         | `hold`              |
| 분포          | 히트맵에서 뽑은 점 둘                    | 기본값                         | —                   |
| 지원          | 공 가진 말의 옆·앞·뒤                    | 빈 공간 · 패스 경로가 열렸는가 | —                   |
| 침투          | 상대 수비 라인 바로 앞                   | 라인 뒤 공간 · 위협(xT)        | `runBehind`         |
| 박스 진입     | 상대 박스 안 세 레인 (마무리 국면)       | 위협(xT) · 빈 공간             | `boxPresence`       |
| 오버래핑 · 폭 | 공 가진 말보다 앞, 자기 측면 터치라인 쪽 | 빈 공간 · 위협(xT)             | `width` · `advance` |

침투는 라인이 높을수록(뒤 공간이 넓을수록), 침투 성향이 클수록 잦다. 멘탈리티는 앞쪽
성분을 무겁게 해 침투·박스 진입 점의 밀도를 올린다.

수비 중인 팀의 말은 **압박**(공 가진 말에게 붙는다), **대인 마크**, **슛 길목**, **존 커버**,
**라인 유지** 중에 고른다. 압박에 나서는 말의 수와 압박을 시작하는 거리는 전술의
압박과 역할의 `press`가 정한다. 압박에 나서지 않는 말의 후보:

| 후보    | 자리                                       | 상황 가치                                  |
| ------- | ------------------------------------------ | ------------------------------------------ |
| 자리    | 형태 자리 (오프사이드 트랩이면 한 걸음 앞) | 기본값                                     |
| 분포    | 히트맵에서 뽑은 점 둘                      | 기본값                                     |
| 마크    | 배정받은 상대의 골 쪽                      | 그 상대의 위협(xT)                         |
| 슛 길목 | 공과 우리 골문을 잇는 선 위                | 공 자리의 슈팅 위협 — 골에 가까울수록 크다 |
| 존 커버 | 자리와 우리 골문 사이, 공 쪽               | 공이 우리 진영에 깊을수록 크다             |

- **마크는 분포로 배정된다.** 매 틱 위협이 큰 상대부터, `거리 ÷ ρ(그 상대의 자리)` —
  가까우면서 그 자리가 제 분포의 무거운 곳인 수비 — 가 가장 작은 말에게 한 명씩 맡긴다.
  상대가 움직여 다른 수비의 값이 충분히 작아지면 마크가 그 말에게 넘어간다. 지금 맡은
  수비에 여유를 줘 틱마다 뒤바뀌지 않는다. 최전방(CF·ST)은 사람을 잡지 않는다.
- **골 쪽 인원** — 공이 우리 진영에 깊을수록 수비 분포의 무게가 박스 쪽 성분으로 옮고,
  슛 길목과 존 커버의 가치가 커진다. 그래서 공이 박스 앞에 오면 압박에 나선 말 뒤로
  길목과 골 앞을 채우는 말이 선다.
...
- **되돌아 뛰기** — 공을 잃었을 때 자리보다 한참 앞에 있는 말은 자리별 속도로 돌아온다
  (수비·중원은 빠르게, 최전방은 걸어서). 한참 멀면 전력이다. 슛 길목·존 커버로 물러서는
  말은 공이 우리 골에 가까울수록 더 서두른다 — 슛보다 먼저 닿아야 막는다.
- **침투 추적** — 마크한 상대가 멀어지면 따라 뛴다. 제 분포에서 멀어질수록 다른 수비에게
  넘어가기 쉽다(위).
...
공격 중에는 **역습 가담**(공을 되찾은 직후 자리보다 뒤에 있는 말이 치고 나간다)이
