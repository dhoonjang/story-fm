...
- **무게가 옮는다.** 성분의 가중치는 공의 깊이 · 국면 · 멘탈리티에 연속으로 반응한다.
  풀백은 수비 성분(수비 라인 옆, 뒤로 자기 박스까지), 가담 성분(측면 높은 곳)과 측면 끝
  성분(상대 마무리 지역의 골라인 근처 — 크로스를 올리는 자리)을 갖고, 공이 전진할수록 앞의
  두 성분이 무거워진다. 측면 끝 성분은 `advance`가 크게 기울여 윙백에게 특히 무겁다.
...
- **표 하나가 모양을 쥔다.** 자리 묶음 × 국면마다의 성분(중심 오프셋 · 퍼짐 · 기본
  가중치 · 가중치가 반응하는 입력)은 `tuning.ts`의 한 표다.
