...
- **왜 게이트가 아닌가.** 한 번 도는 데 수십 분이고, `ci.yml`의 초록까지의 벽시계는
  지금 4분대다(그 파일의 측정 주석). 하네스를 거기 얹으면 PR마다 그 몇 배를 물고,
  정작 밴드를 움직이는 커밋은 드물다. 게이트가 지키는 것은 **고정 기대값의 회귀**이고
  (AGENTS.md §5) 밴드는 그런 값이 아니다.
