...
카드의 사실 문장에는 이름이 그대로 박히고(`${name} …`), 그 문장은 화면의 키포인트
...
건 것과 반려한 것, 구간이 구른 패킷과 난수 채널, 사건, 그 사이 굴러간 하루하루가 턴
...
([match.md](./simulation/match.md) §8). **협상 방**도 같은 갈림이다 — 감독이 자리에 앉는
...
- **이름은 손잡이다.** 장부와 이야기에 선 선수 이름을 누르면 그 선수의 카드가
  ...
  카드를 연다 ([match.md](./simulation/match.md) §8).
...
- **답을 기다리는 일은 상단 띠가 든다** — 회견 · 면담(찾아온 사람) · 도착한 편지 ·
  답할 차례의 협상 · 기한이 다가온 약속. **다섯 갈래가 전부고, 있을 때만 선다** —
  하나도 없으면 띠에 아무것도 서지 않는다(PSR·부채가 없으면 서지 않는 재정 노트와 같은
  규약). 한 갈래에 하나면 그 이름이 서고 여럿이면 **수 하나로 접는다**. 남은 날은 기한이
  날짜로 서는 갈래(약속)만 든다 — 나머지 넷은 서 있다는 것 자체가 기한이다.
  - 값은 GM 스냅샷이 읽는 **같은 코어 함수**에서 뷰 하나(`views.attention`)로 온다 —
    `pendingPress` · `pendingApproach` · `arrivedResponses` · `pendingVerdicts` ·
    `duePromises`. 두 자리가 다른 말을 하지 않는다.
  - **편지와 협상은 겹치지 않는다.** 답이 도착해 다음 호흡에 저절로 열릴 것은 편지고,
    감독이 도구로 답해야 하는 것만 협상이다. 코어의 두 함수는 같은 협상을 함께 든다
    (도착한 답은 판정 대기이기도 하다) — 그대로 세우면 협상 하나가 칩 둘이 되므로,
    협상 쪽에서 편지를 뺀다.
  ...
  - **읽는 값이라 손잡이가 아니다** — 탭 정지점도 포커스 링도 갖지 않고 `aria-label`로만
    읽힌다(판세의 9칸 격자·누적 xG 계단선과 같은 규약). 채팅으로 돌아가는 문은 아이콘
...
- **읽는 값과 조작 대상은 생김새도 다루는 법도 다르다.** 눌러서 무언가 열리거나
  바뀌는 것만 탭 정지점과 포커스 링을 갖는다 — 판세의 9칸 격자처럼 읽기만 하는
