...
| 낱말            | 무엇                                                                                        | 예                                                                |
| --------------- | ------------------------------------------------------------------------------------------- | ----------------------------------------------------------------- |
| **스킬**        | **LLM이 직접 부르는 도구.** 카탈로그가 이름과 설명을 갖고, 호출이 칩·카드로 남는다          | 평시 GM 24 · 매치 GM 3                                            |
| **출력 스키마** | 에이전트 호출 하나가 내는 **JSON의 꼴.** 요청이 응답의 꼴로 강제한다 — 도구가 아니다        | 해석기의 `{ ops, unresolved }` · 경기 마감의 `SETTLE_MATCH_INPUT` |
...
2. **경기 결과도 코어가 정한다.** 감독의 경기는 [실시간 경기](simulation/live-match.md)의
   말들이, 다른 구장은 [간이 시뮬](simulation/match.md)이 사건을 확정하고 LLM은 중계·연출로
...
| 골의 원인   | `EventCause[]` — 행동의 사슬 코드 + 선수 (전술 XP의 근거) | 중계 (매치 GM) · 경기 리포트                                                                   |
...
조사는 `packages/domain/src/josa.ts`의 `josa(앞말, "이/가")` 하나가 고른다 — 앞말의
...
하나의 타임라인에 모델 호출과 나란히 순서대로 쌓인다(`journal`, [llm/models.md](llm/models.md)
...
| 다음 말   | `<suggest_reply>…</suggest_reply>` — 모델 턴의 마지막 줄. GM이 감독이 이어 할 말 하나를 내고, 코어가 꺼내 입력창의 placeholder로 세운다 — Tab·→가 입력으로 받는다 ([ui/design-system.md](ui/design-system.md) §6) | `<suggest_reply>오전 훈련부터 보자</suggest_reply>` |
...
- **제안은 감독의 말이 아니다.** GM이 낸 다음 말 한 줄은 세이브의 턴(`ChatTurn.suggestion`)에만
  ...
  보내든, **보낸 것만** 감독의 말이 된다 ([llm/agents.md](llm/agents.md) §2).
...
flowchart LR
  ...
  P -->|경기일| Q["경기<br/>말 22개가 구른다 → 체크포인트 검증 → 반영<br/>(live-match.md · match.md)"]
  Q --> P
  P -->|마주 앉는다| N["협상 방<br/>서류 → 말 → 앵커 ± 한도<br/>(transfer.md §12-2)"]
  N --> P
...
감독 경질, 감독이 나서지 않은 협상 라운드의 판정, 기자회견 개최. 감독의 결정이
필요한 일 앞에서만 멈춘다.
...
([match.md](./simulation/match.md) §9). **협상 방**도 같은 갈림이다 — 감독이 자리에 앉는
게이트를 지나면 채팅 옆에 **조건서 · 인내 · 건너편**이 서고, 채팅은 그 협상의 턴만 흐른다
([transfer.md](./simulation/transfer.md) §12-2 · [ui/design-system.md](./ui/design-system.md) §7-1).
...
| 재정   | 월간 보고서·원장·PSR                                                                                                                                                                                                                                                                                                                      |
| 대회   | 대회별 순위표(진출권·강등 구역 · **전체/홈/원정** · 최근 5경기 폼)·**개인 순위**(득점·도움·평점·클린시트·징계)와 팀 열·라운드 일정·브래킷·**그 대회의** 다음 경기 (경기 중 탭은 팀의 다음 경기 — [match.md](./simulation/match.md) §9.2) · **역대**(시즌을 골라 그 시즌의 최종 순위표·우승·시상 — [season.md](./simulation/season.md) §6) |
...
- **이름은 손잡이다.** 장부와 이야기에 선 선수 이름을 누르면 그 선수의 카드가
  ...
  카드를 연다 ([match.md](./simulation/match.md) §9).
  카드는 매 턴 짐에 실리지 않고 **열 때 하나씩** 오고, 남의 구단 선수는 안개를
  통과한 값만 싣는다 ([data/player.md](./data/player.md) §9.5). 손잡이가 서려면 뷰
  행이 이름과 함께 **id**를 들어야 한다 — 이름으로 선수를 되찾는 화면은 동명이인에서
  갈린다.
- **제안은 제안서로도 낸다** — 선수 카드의 「제안」과 채팅의 시장 카드(오퍼·답·재계약)와
  협상 방의 「제안」이 같은 폼을 연다. 이적료·분할·주급·연수·지위와 조건(바이아웃·보너스·
  공격 포인트 보너스·인상 조항·추가 영입·주장·등번호)을 정확한 값으로 적고 「제안서 작성」을
  누르면 **제안서가 입력창에 첨부된다** — 감독은 말을 덧붙여 보내거나 말 없이 보낸다. 표
  밖의 조건과 한마디는 말이다. 폼이 미리 채우는 값은 전부 코어가 낸 자
  (`PlayerCardView.proposal` — 요구가·기대 주급·예산·주급 여력)이고, 카드에서 열면 그
  카드의 조정안이 출발점이다. 보내는 길은 **턴 하나**다: 서버가 구조체를 손잡이로 받아
  코어 명령을 턴 앞에서 걸고(`applyProposal`), 카드가 있는 명령(오퍼·재계약)은 그 턴에
  카드로 서고 카드가 없는 명령(개인 조건 선제안·조건만)은 제안 줄로만 남으며 — 명령 이름이
  칩으로 서지 않는다 —, GM에게는 「화면에서 제안했다」가 오퍼레이터 봉투로 간다. 코어가 반려하면 턴은 돌지 않고 그 이유가 배너에
  서며 제안서는 첨부된 채 남는다 — 감독이 값을 고친다
  ([simulation/transfer.md](./simulation/transfer.md) §12-3).
- **답을 기다리는 일은 상단 띠가 든다** — 회견 · 면담(찾아온 사람) · 답할 차례의 협상 ·
  기한이 다가온 약속. **네 갈래가 전부고, 있을 때만 선다** —
  하나도 없으면 띠에 아무것도 서지 않는다(PSR·부채가 없으면 서지 않는 재정 노트와 같은
  규약). 한 갈래에 하나면 그 이름이 서고 여럿이면 **수 하나로 접는다**. 남은 날은 기한이
  날짜로 서는 갈래(약속)만 든다 — 나머지 셋은 서 있다는 것 자체가 기한이다.
  - 값은 GM 스냅샷이 읽는 **같은 코어 함수**에서 뷰 하나(`views.attention`)로 온다 —
    `pendingPress` · `pendingApproach` · `pendingVerdicts` · `duePromises`. 두 자리가
    다른 말을 하지 않는다.
  - **띠에 서는 협상은 감독이 도구로 답할 것뿐이다.** 답할 날이 된 라운드는 그날의 tick이
    앵커로 굳히고 결과가 사건 한 줄로 서므로(transfer.md §12-1), 띠가 드는 것은 그 뒤에
    감독의 차례로 남은 자리다 — **상대가 되부른 조정** · 들어온 오퍼 · 확정을 기다리는
    합의 · 되부른 개인 조건(`pendingVerdicts`). 우리 오퍼에 온 답 그 자체는 감독이 할
    일이 없어 서지 않는다.
  - **경기 중에도 협상 방 안에서도 서지 않는다** — 90분은 답할 자리가 아니고 그 칸은
    경기 시계의 것이며, 방 안의 안건은 건너편 한 사람이다.
  - **읽는 값이라 손잡이가 아니다** — 탭 정지점도 포커스 링도 갖지 않고 `aria-label`로만
    읽힌다(누적 xG 계단선과 같은 규약). 채팅으로 돌아가는 문은 아이콘
    줄에 이미 서 있어, 같은 문을 두 벌로 두면 읽는 값과 조작 대상의 생김새가 섞인다.
...
- 관계 등급 — 사이를 옮기는 자리가 이력 압축 하나뿐이라, 코어가 저절로 세우는 라커룸
  파벌은 없다. 서사 아크의 갈래도 장부 한 줄로 열리는 다섯까지다
  ([data/people.md](data/people.md) §10)
- 스태프의 숫자 효과와 구장 — 코치·의료진·스카우트는 이름·계약·고용 손잡이를 갖고
  서지만([data/people.md](data/people.md) §2-2) 훈련 흡수율·회복 속도·관측 오차 중
  어느 것도 움직이지 않는다. 구장 수용 인원은 시드 고정값이라 증설 말고는 투자 결정이
  없다 ([simulation/finance.md](simulation/finance.md) §12)
- 오프시즌 연출 — 은퇴는 1월 예고·마지막 홈경기·명부로 서지만 시상식의 자리가 없고,
  은퇴한 사람이 스태프 풀에 들어오는 길도 없다 — 풀은 그해 여름의 추첨이 채운다
...
|                | 문서                                                                 | 다루는 것                                                                |
| -------------- | -------------------------------------------------------------------- | ------------------------------------------------------------------------ |
| **데이터**     | [data/game-state.md](data/game-state.md)                             | 2-레이어 · 엔티티와 관계 · 파생 · 세이브 정책                            |
...
| **시뮬레이션** | [simulation/live-match.md](simulation/live-match.md)                 | 실시간 경기 — 말의 규칙 · 전술 · 시트 · 부하 · 클라이언트 시뮬과 검증    |
|                | [simulation/match.md](simulation/match.md)                           | 경기 능력 · 장부 · 벤치 · 경기 후 반영 · 간이 시뮬 · 상수와 근거         |
|                | [simulation/football-reference.md](simulation/football-reference.md) | 실제 축구 통계 — 밸런스의 기준점                                         |
...
|                | [llm/agents.md](llm/agents.md)                                       | 열세 에이전트의 계약 · 결산과 교섭 · 입력 층 · 스냅샷 · 조회             |
