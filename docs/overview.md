...
- **이름은 손잡이다.** 장부와 이야기에 선 선수 이름을 누르면 그 선수의 카드가
  ...
  갈린다.
- **제안은 폼으로도 낸다** — 선수 카드의 「제안」과 채팅의 시장 카드(오퍼·답·재계약)의
  「제안」이 같은 폼을 연다. 이적료·분할·주급·연수·지위와 조건서(바이아웃·보너스·인상
  조항·추가 영입·주장·등번호·그 밖)를 정확한 값으로 적고, 한마디를 함께 보낼 수 있다.
  폼이 미리 채우는 값은 전부 코어가 낸 자(`PlayerCardView.proposal` — 요구가·기대 주급·
  예산·주급 여력)이고, 카드에서 열면 그 카드의 조정안이 출발점이다. 보내는 길은 **턴
  하나**다: 서버가 구조체를 손잡이로 받아 코어 명령을 턴 앞에서 걸고(`applyProposal`),
  그 카드는 그 턴에 서며, GM에게는 「화면에서 제안했다」가 오퍼레이터 봉투로 간다.
  코어가 반려하면 턴은 돌지 않고 그 이유가 폼에 선다 — 감독이 값을 고친다
