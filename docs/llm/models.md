...
- **교섭 상대가 제 키를 갖는 이유는 읽는 것이 다르기 때문**이다 — 평시 GM은 감독이 한
  ...
  부른다. 협상 방의 `negotiation-gm`은 이 협상의 서류·상황·방의 대화만 읽고 메인 채팅을
  읽지 않으며, 자르는 것은 코어다 (agents.md §4-1). 감독이 나서지 않은 라운드에는 그 자리가
  아예 서지 않는다 — 코어가 앵커로 굳힌다. 협상 GM이 매치 GM과 같은 자리인 이유는 같다 —
