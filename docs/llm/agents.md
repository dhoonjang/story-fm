...
**산출이 도구 하나뿐인 호출은 같은 길이다** — 훈련 결산(`report_training`)도 여기 든다.
...
입구는 넷뿐이다 (`core/player-ref.ts`) — 우리 선수는 `pickOurPlayer`, 남의 선수는
`pickRivalPlayer`, 어느 쪽이든 되는 자리만 `pickAnyPlayer`, 회견 사실 카드처럼 이미
좁혀진 목록은 `pickPlayerAmong`. **자격이 정해진 명령은 그 자격의 문으로 든다** —
