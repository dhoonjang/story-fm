...
| ③    | `tactic_orders` 도구  | 감독의 지시를 **지시 해석**(LLM)이 `TacticOrders`로 옮기고 코어가 명령으로 적용 → 패킷 재계산 → 새 판이 도구 결과로. 시계는 그대로 |
...
`tactic_orders`·`advance_match`의 핸들러가 부른다. 산출은 `TacticOrders` JSON 하나이고
계약은 그대로다:

- **산출 도구는 요청이 강제한다.** `report_tactic_orders`는 `toolChoice`로 실린다
  ([models.md](./models.md) §3-2). 도구가 안 불린 응답은 실패로 세어 한 번 더 부른다.
- **산출은 도구 호출이 아니라 JSON 하나**다 (`TacticOrdersSchema`). 교체·6축과 토글 넷·
  개인 지시·지역 플랜·공략·세트피스 키커와 인원·대화가 한 객체의 필드로 함께 온다.
  `advance`는 더 읽지 않는다 — 진행은 GM이 어느 도구를 불렀는가다.
