...
**경기를 바꾸는 도구는 없다.** 매치 GM이 쥔 둘(`tactic_orders`·`finalize_match`)은 코어를
부르는 손잡이다 — 지시는 판독기가 JSON으로 옮긴 것을 코어가 실재 확인과 한도를 거쳐
적용하고, 경기는 말의 규칙이 굴리고([../simulation/live-match.md](../simulation/live-match.md)),
마감은 코어가 앵커를 박는다.
그 뒤의 두 에이전트가 내는 것은 **출력 스키마** 하나뿐이다 — 판독기의 `{ ops, points, sheet, unresolved }`,
마감의 결산 JSON. 훈련 결산과 같은 자리로, 요청이 응답의 꼴로 강제할 뿐 **도구가 아니라
그 호출이 내는 JSON의 꼴이다**(§0). 경기 중 고정층은 둘뿐이라 평시(24개)보다 훨씬
작다(§5).
...
## 3. 경기 — 매치 GM이 도구로 지시를 판에 올린다

경기 턴은 **호출 하나**다. 매치 GM이 이 경기의 이력 전부를 쥔 채 감독의 말에 반응하고,
판을 움직여야 할 때만 도구를 부른다. **시계는 GM의 것이 아니다** — 경기 시계는 클라이언트의
실행기가 밀고, 감독이 입력하거나 턴이 도는 동안에는 x1로 흐른다([../simulation/live-match.md](../simulation/live-match.md) §8).
도구 뒤에는 두 에이전트가 선다 — 경기를 읽어 전술 포인트와 시트를 쓰고 감독의 말을 그 판독
위에서 명령 인자로 옮기는 **판독기**와, 끝난 경기를 결산하고 마무리 중계를 쓰는 **경기 마감**.
둘 다 GM의 도구 핸들러 안에서 돌고, 그 결과가 **도구 결과로 같은 호출에 돌아온다** — 그래서
GM은 자기가 방금 바꾼 판을 읽고 쓴다.

| 걸음 | 누가                  | 무슨 일                                                                                                                                                                                                                                                                                       |
| ---- | --------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ①    | 감독 · 실행기         | 감독이 말한다 — 그 순간 경기가 멈추고 클라이언트가 체크포인트를 제출한다. 또는 정지점(골·경고·퇴장·부상·교체·상대 벤치의 전술 전환·하프의 끝·종료 휘슬)이 체크포인트로 확정되면 실행기가 시계를 잡고 **정지점 턴**(`match_stop`)을 연다 — 판독기가 먼저 그 사건으로 포인트와 시트를 다시 쓴다 |
| ②    | **매치 GM** (LLM)     | 이력 전부 + 장부 + 지난 턴 뒤의 사건(`<events>`)을 읽고 답한다. 정지점 턴은 그 사건의 중계다. 말만 건 턴은 도구 없이 장면만                                                                                                                                                                   |
| ③    | `tactic_orders` 도구  | 이번 턴 감독의 말(코어가 넘긴다)을 **판독기**(LLM)가 지금 전술 포인트 위에서 읽어 `ops`·새 포인트·시트로 옮기고, 코어가 검증해 **멈춘 tick에 적용할 것**으로 돌려준다 → 새 판이 도구 결과로. 한 턴에 한 번                                                                                    |
| ④    | `finalize_match` 도구 | 장부가 `finished`일 때만 — 마감(앵커) → **경기 마감 에이전트**(LLM)가 결산과 마무리 중계를 쓴다 → 도구 결과로                                                                                                                                                                                 |
| ⑤    | **매치 GM**           | 도구 결과를 읽고 중계·연출·벤치 대화를 쓴다. 턴이 끝나면 경기가 재개된다                                                                                                                                                                                                                      |

**경기를 바꾸는 것은 코어뿐이다.** GM이 쥔 도구 둘 중 어느 것도 사건을 만들거나 스코어를
정하지 않는다 — 사건은 말의 규칙이 만들고, 지시는 판독기가 JSON으로 옮긴 것을 코어가 실재
확인과 한도를 거쳐 적용한다. "진행할지"를 GM이 정할 자리도 없다 — 경기는 감독이 말을 멈추면
구르고, 정지점 턴이 끝나면 다시 구른다. **중계는 끊기지 않는다** — 골과 카드와 하프의 끝은
감독이 말을 걸지 않아도 그 자리에서 중계된다. 종료 휘슬의 정지점 턴이 마감을 부른다.

### 매치 GM (`match-gm`)

- **읽는 것** — `MATCH_GM_SYSTEM` · 레퍼런스(구단·감독·수석코치 카드·`<pre_match>`) ·
  이 경기의 중계 이력 전부(`casterHistory`) · 이번 턴 감독의 말 · `<events>`(지난 턴 뒤 장부에
  앉은 사건 — 골·슛·카드·교체·부상·상대 전환) · `<ledger>`·`<standing>`·`<match_state>`(점유·
  슈팅·xG·패스·부하의 지금까지 합) · `<points>`(감독의 분석이 허락한 전술 포인트 —
  [../simulation/live-match.md](../simulation/live-match.md) §6.2).
- **부르는 것** — 감독이 지시한 턴에 `tactic_orders` 한 번(판이 서고 GM은 그 판을 미리
  읽는다). 이름을 부르거나 말만 건 턴, 정지점 턴은 도구 없이. 장부가 `finished`면
  `finalize_match`.
  규칙은 도구 설명이 갖는다 ([prompts.md](./prompts.md) §5-2). 장면의 마지막 줄은 평시와
  같은 제안 한 줄이다(§2) — 경기 이력은 제공자 원형이라 그 줄이 지난 중계 본문에 남는데,
  그것은 모델 자신의 출력이고 감독의 말로 서는 자리는 없다.
- **감독의 말은 코어가 넘긴다.** `tactic_orders`는 인자가 없다 — 이번 턴 감독의 발화
  ...
  반려한다.
- **「70분에 라야 빼」는 예약이 아니다.** 예약 지시 장부도 조건 지시도 없다 — 시계는 감독이
  보고 있고, 그 분에 감독이 멈춰 말하면 된다. GM은 그 사실을 픽션 안에서 말한다.
- **재시도의 자국은 평시와 같다** — 도구가 돌았거나 델타가 나갔으면 다시 부르지
  않는다. 중계가 깨지면 그 턴은 실패로 올라가고 세이브는 그대로다(§8) — 경기는 마지막
  확정 상태에서 이어진다.
- **장면의 시각은 장부가 준다** — 멈춘 그 tick의 분이다. 스트리밍의 첫 줄도 첫 델타가
  나가는 순간의 장부를 읽는다.
- 화면에 세우는 골·카드 표식은 **장부의 사건**에서 만든다 — 중계 문장을 되읽지 않는다.
- **`<events>`는 역할을 이름 옆에 적는다** — 골은 `득점 X · 도움 Y`, 교체는 `OUT X · IN Y`.
- **골 줄은 그 골 뒤의 스코어를 함께 적는다** — `골 (아스널 1 – 0 첼시)`. 중계의 골 문형이
  그 한 줄에서 나오고([prompts.md](./prompts.md) §1), 지난 턴 뒤 골이 둘이면 중간 스코어를
  중계가 세지 않아도 된다.
- **슛 줄은 결과와 큰 기회 여부를 적는다** — `슛 — 큰 기회, 선방에 막혔다`. 장부는 슛마다
  ...
  [match.md](../simulation/match.md) §9.3). xG 자체는 싣지 않는다 — 화자가 입에 담을 수 없는
  수치다.
- **골의 원인도 한 줄로 온다** — `EventCause`를 렌더러가 문장으로 옮긴 것이다(스루패스로
  라인 뒤를 뚫었다 · 코너에서 · 상대 진영에서 뺏어). 중계는 장면을 지어내지 않고 일어난
  사슬을 옮긴다.
- 경기 중에도 시계는 흐르되 **날짜는 막는다**. **정지점은 감독의 차례다.**

### 판독기 (`match-reader`) — 경기를 읽고, 지시를 그 판독 위에서 옮긴다

전술 포인트와 시트의 저자다 ([../simulation/live-match.md](../simulation/live-match.md) §6.2).
**세계의 눈**이라 안개가 없다 — 양 팀의 진짜 능력치, 지금까지의 경기 통계(점유·슈팅·xG·
패스의 레인 분포·압박과 경합·선수별 부하와 체력), 지난 정지점 뒤의 사건, 카드, 경기 전
서사, 그리고 지금 전술 포인트를 읽는다. 문장은 쓰지 않고 감독에게 보이지 않는다. 산출은
**출력 스키마 JSON 하나**다 (`outputSchema` — [models.md](./models.md) §3-2):
...
**도는 때는 넷이다.** 넷 다 경기 시계가 멈춘 자리다.

| 때         | 부르는 곳                              | 읽는 것                                                       | 내는 것                |
| ---------- | -------------------------------------- | ------------------------------------------------------------- | ---------------------- |
| 킥오프     | `start_match` (①)                      | 경기 전 사실(`<pre_match>` · 서사) · 양 팀 명단과 전술        | 첫 포인트·시트         |
| 지시 턴    | `tactic_orders` 핸들러                 | 위 + 지금 포인트 + `<match_state>` + `<match_log>` + `@감독:` | `ops` · 새 포인트·시트 |
| 골·퇴장 뒤 | 체크포인트 확정 — 재시작으로 멈춘 시간 | 위 + 지금 포인트 + `<events>`                                 | 새 포인트·시트         |
| 하프타임   | 체크포인트 확정                        | 같음                                                          | 새 포인트·시트         |

- **말이 구르는 동안에는 돌지 않는다.** 시트는 정지점에서만 바뀌고, 그 사이 말의 규칙은
  같은 시트를 읽는다 — (시드, 경기, tick, 입력 로그)에 대한 결정성이 여기서 선다. 판독의
  결과는 tick 도장을 받아 입력 로그에 남고, 검증 재실행이 같은 tick에 같은 시트를 넣는다.
- **정지점 뒤에 부르는 것은 판이 사건으로 바뀌기 때문이다.** 파울 세 번 당한 미드필더,
...
- **상대 벤치의 작은 수도 판독기가 쓴다.** 상대 센터백이 우리 윙어를 따라붙는 것은 AI
  벤치 코드가 아니라 판독이다(`behavior`) — 코드에 남는 것은 6축 이동·모양·교체뿐이다
  ([../simulation/match.md](../simulation/match.md) §3.3). AI 감독의 등급은 입력에 사실로 실린다.
  **LLM이 고르고 코어가 값을 매긴다.** 시트 줄은 누구를·어느 레인을·어느 쪽으로·얼마나(step
  1\~3)까지만이고, 실재 확인·한도·예산·대가·소화율은 전부 코어다(live-match.md §6.2).
...
판도 포인트도 그대로다. 정지점 뒤의 실패는 삼킨다 — 지난 포인트·시트가 그대로 다음의
...
같은 자리이고(§8), 정지점 뒤에는 고정 포인트 한 벌을 낸다. 실모드와 다른 것은 저자뿐이다.
...
경기 시계는 클라이언트의 실행기가 민다. 턴은 서버의 확정 tick 위에서 돌고, 그동안 화면의
경기는 x1로 이어지다가 턴이 끝나면 지시가 반영된 확정 상태에서 다시 굴려 따라잡는다
([../simulation/live-match.md](../simulation/live-match.md) §8.3). 말로 시계를 흘려보낼 길은
없다 — 턴이 시계를 미는 일은 없다.

**킥오프 턴은 도구가 없다.** 감독이 경기장에 들어선 그 한 턴은 첫 휘슬만 연다 — 이력은
경기 전 대화(`relevantTurns`), 도구도 사건도 없다. 공이 구르는 것은 그다음부터다.
...
- **평은 보고서에 남는다** (`ScoutReport.verdict`). 카드는 한 번 지나가지만 모달은
  ...
  ([../data/player.md](../data/player.md) §9.5). 실패하면 비어 있어 선택이다.
...
- **`<opponent>`는 경기 전날과 당일에만 선다** — 코어의 상대 분석 리포트가 그대로 온다
  (→ [../simulation/match.md](../simulation/match.md) §3.6). 감독이 라인업을 정하는
...
않으면 가장 최근에 끝난 우리 경기다 (→ [../simulation/match.md](../simulation/match.md) §9.3).
...
다음 우리 경기다 (→ [../simulation/match.md](../simulation/match.md) §3.6).
...
밸런스에는 성장 편향이 생긴다 ([../simulation/match.md](../simulation/match.md) §8.6).
