...
실행기가 밀고, 감독이 입력하면 멈춘다([../simulation/live-match.md](../simulation/live-match.md) §8).
...
경기 시계는 클라이언트의 실행기가 밀고, 감독이 입력을 시작한 순간 멈춘다. 턴이 도는 동안
공은 멈춰 있고, 턴이 끝나면 다시 구른다 — 대화 한 마디에 시간이 흐르지 않고, 반대로 말로
시계를 흘려보낼 길도 없다. 공이 멈춰 있을 때 감독이 말하는 것은 공짜여야 한다.
