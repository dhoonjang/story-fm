...
**하는 일도 설정 키도 열한 갈래다.** 평시의 GM, 경기의 매치 GM, 도구 뒤에서 감독의 말을
...
- **실패하면 사실 줄만 남는다** — 평이 없는 보고서는 안개 한 줄로 닫힌다. 훈련 결산과
...
입찰) · `<characters>`(그 자리의 사람들 — 선수 본인과 대리인)가 서고, `<anchor>`(인내 ·
확률·기준 판정·고를 수 있는 판정·구간·기한)는 대화 뒤에 따로 선다. ⚠️ **고정층에 이번 협상의 숫자를 넣지 않는다**
...
agent "조르제 멘데스" — 주급 · 계약 지위 · 등번호 · 설득 논거의 답
</voices>
```
...
| 블록             | 무엇                                                                                                                                                                                                                                                             | 바뀌는 때    |
| ---------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------ |
| `<counterparty>` | 이 협상의 서류 — 갈래·양쪽·**누가 무엇을 답하나**·선수의 사실·오퍼 이력·값의 자·확인된 논거·인물지                                                                                                                                                               | 라운드마다   |
...
올라 있을 때만) · 태도(`stance`) · **화자가 붙은 말 줄**(`lines` — `<voices>`에 선 화자
하나에 한 줄, 최대 둘). 코어가 논거를 사실 대조하고, 말투와 거짓으로 인내를 깎고,
판정을 앵커 ± 한도로 잘라 반영한다(`settleTableReply`). 말은 장부를 움직이지 못한다 —
...
- **보고서가 서는 것을 판정이 막지 않는다.** 스카우팅 평이 실패해도 카드는 사실 그대로
