...
| 지시 해석 (`apply-orders.ts`)       | `apply-orders`      | 두 GM의 `apply_orders` 뒤 — 감독의 판·운영 지시를 **구조화된 의도 하나**로 | 도구가 반려로 답한다 — 판은 그대로                     |
| 시장 해석 (`market-orders.ts`)      | `market-orders`     | GM의 `market_orders` 뒤 — 이적·재정·감독직 지시를 **스킬 인자**로          | 도구가 반려로 답한다 — 장부는 그대로                   |
...
`llm.yml`의 항목은 이 아홉과 같다 — 첫 장면은 `gm`의 키를 쓴다. 협상 상대는 편지든
...
**경기를 바꾸는 도구는 없다.** 매치 GM이 쥔 셋(`apply_orders`·`advance_match`·
`finalize_match`)은 코어를 부르는 손잡이다 — 지시는 해석이 JSON으로 옮긴 것을 코어가
실재 확인을 거쳐 적용하고, 구간은 코어의 시뮬이 굴리고, 마감은 코어가 앵커를 박는다.
그 뒤의 두 에이전트가 내는 것은 산출 도구 하나뿐이다 — 해석의 `report_intent`, 마감의
`settle_match`. 산출 도구는 훈련 결산의 `report_training`과 같은 자리로, **구조화 출력을
강제하는 그릇이지 스킬이 아니다.** 경기 중 고정층은 셋뿐이라 평시(24개)보다 훨씬
작다(§5).

**평시 GM은 받아쓰기 인자를 채우지 않는다.** GM이 쥐는 것은 판정이 드는 도구(대화·
사건·회견·면담·재정 이벤트)와 조회, 그리고 원문을 넘기는 손잡이 셋 — `apply_orders`
(판과 선수단 운영: 라인업·1·2군·6축·개인 지시·세트피스·지역 플랜·공략·완장·훈련·집중
육성·멘토·2군 방침·등번호·유스 계약) · `market_orders`(이적·재정·감독직: 오퍼·답·확정·
철회·재계약·해지·리스트·이적 요청·되사기·임대 복귀·예산·보드 요청·사재·표값·감독직
수락·흥정·지원) · `speak_at_table`(협상 상대에게 하는 말)이다. 감독의 말을 원문 그대로
넘기면 도구 뒤의 해석기가 스킬의 인자를 채우고 코어가 적용한다 — 인자가 값 몇 개라도
장면을 쓰는 머리가 같은 왕복에서 채우면 하나를 빠뜨리고, 도구 스펙이 고정층의 절반을
먹는다. 해석기가 채우는 스킬의 스키마는 **도구 정의에서 그대로 묶는다**(`orders-ops.ts`
— 스킬 이름 아래 그 도구의 입력 스키마를 배열로). 한 벌이라 갈릴 데가 없고, 검증도 그
도구의 Zod가 한다.
...
- **전술은 해석기가 옮긴다** — `apply_orders(orders)`의 핸들러 뒤에서 지시 해석이
...
| ③    | `apply_orders` 도구   | 감독의 지시를 **지시 해석**(LLM)이 `Orders`로 옮기고 코어가 스킬로 적용 → 패킷 재계산 → 새 판이 도구 결과로. 시계는 그대로 |
...
- **읽는 것** — `MATCH_GM_SYSTEM` · 레퍼런스(구단·감독·수석코치 카드·`<pre_match>`) ·
  ...
  새 패킷**이 함께 온다.
- **부르는 것** — 지시가 나올 때마다 `apply_orders`(판과 패킷이 서고 GM은 그 판을
...
### 지시 해석 (`apply-orders`) — 도구 뒤의 분류기

`apply_orders`·`advance_match`의 핸들러가 부른다. 산출은 `Orders` JSON 하나이고
계약은 그대로다:

- **산출 도구는 요청이 강제한다.** `report_intent`는 `toolChoice`로 실린다
  ([models.md](./models.md) §3-2). 도구가 안 불린 응답은 실패로 세어 한 번 더 부른다.
- **산출은 스킬 호출이 아니라 JSON 하나**다 (`OrdersSchema`). 교체·6축과 토글 넷·
...
- **이 경기의 지난 턴 전부를 읽는다** — `<match_log>`(중계와 감독의 말, 턴당
  `ORDERS_TURN_CHARS`에서 자른다). 감독의 지시는 앞 턴의 대화를 잇는 말일 때가
...
- **훈련 브리프의 대화는 장면이지 코치의 말이 아니다.** 모델 턴의 화자는 「장면」으로
  ...
  (`turnFactLines` — 스킬 호출의 요약과 코어 기록을 `[장부] …` 줄로) 이 말하고, 압축
...
선수의 심경 한 줄은 **그 선수와 있었던 일을 쓴 스킬이 함께 남긴다.** 별도 결산은 없다 —
그 대화를 쥔 것은 GM이고, 한 문장을 쓰기에 가장 맞는 자리는 그 면담 스킬 호출 자체다.
...
- **검사는 코어의 것이고 그대로다** (`applyMoodNotes` — [../data/people.md](../data/people.md)
  ...
  이긴다(`MOOD_NOTE_DAYS`). 대상은 **그 스킬이 닿은 선수**로 코어가 좁힌다 — 면담의
  문장이 다른 선수에게 서지 않는다.
...
- **경기 → 평시**: `matchDigest` — 직전 3일 안의 경기 결과·득점·최고 평점 둘에 **라커룸의
  결과**(그 경기의 팀토크 자리와 판정 — 그 턴의 스킬 기록에서)·**퇴장·부상·교체로 나간
...
- **압축은 기억의 주 저자다.** 창이 접히는 순간이 그 구간을 마지막으로 아는 순간이라,
  ...
  턴마다 그 턴의 스킬 호출 요약과 코어 기록(시간 경과·경기 진행)이 `[장부] …` 줄로
...
- ⚠️ **다시 불러 달라질 실패만 다시 부른다.** 도구를 부르지 않은 응답, 상한에 걸려
  ...
  ⚠️ 해석(`apply-orders`)도 같다: 시한을 넘긴 호출을 "다시 말씀해 주세요"로 바꾸면
