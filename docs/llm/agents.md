...
- **세 가지를 낸다** — 지갑(앵커 ±40%) · **능력치의 결**(축마다 ±`ATTRIBUTE_JUDGE_BAND` 8,
  ...
  **시작 사건**(셋까지 — 갈래는 보드·라커룸·언론·개인사, 걸린 사람은 `<club>`의 id만,
  기한은 코어가 `OPENING_DAYS` 45일로 박는다). 시작 사건은 스냅샷의 `<openings>`로 매
  턴 서고, **감독이 그 실마리에 걸린 일을 하면 코어가 닫는다** — 아무도 손대지 않은
  것만 기한이 닫는다 ([../simulation/career.md](../simulation/career.md) §1).
