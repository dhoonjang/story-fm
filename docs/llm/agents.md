...
**하는 일은 아홉 갈래이고 설정 키는 여덟이다.** 평시의 GM, 경기의 매치 GM과 그 도구
뒤의 지시 해석·경기 마감, 협상 테이블 건너편에 앉는 상대, 사람이 읽지 않는 훈련 결산,
밀려나는 이력을 요약으로 옮기는 압축, 그리고 새 게임의 배경을 한 번 읽는 온보딩 판정.
어느 쪽도 상태를 직접 바꾸지 못한다 — 스킬 호출과 앵커 ± 한도만이 통로다.
...
| 지시 해석 (`match-intent.ts`)       | `match-intent`      | 매치 GM의 도구 뒤 — 감독의 말을 **구조화된 의도 하나**로           | 도구가 반려로 답한다 — 판은 그대로           |
...
`llm.yml`의 항목은 이 여덟과 같다 — 첫 장면은 `gm`의 키를 쓴다.

라우팅은 `state.phase` 하나다 — `match`면 매치 GM, 아니면 GM이고, 둘 다 호출 하나다. `phase`는 **라우팅 전용**이라 모델 입력에는 넣지 않는다. 평시 턴의 GM
...
강제하는 그릇이지 스킬이 아니다.** 경기 중 고정층은 셋뿐이라 평시(56개)보다 훨씬
작다(§5).
...
| ③    | `apply_orders` 도구   | 감독의 지시를 **지시 해석**(LLM)이 `MatchIntent`로 옮기고 코어가 스킬로 적용 → 패킷 재계산 → 새 판이 도구 결과로. 시계는 그대로 |
...
### 지시 해석 (`match-intent`) — 도구 뒤의 분류기

`apply_orders`·`advance_match`의 핸들러가 부른다. 산출은 `MatchIntent` JSON 하나이고
계약은 그대로다:

- **산출 도구는 요청이 강제한다.** `report_intent`는 `toolChoice`로 실린다
  ([models.md](./models.md) §3-2). 도구가 안 불린 응답은 실패로 세어 한 번 더 부른다.
- **산출은 스킬 호출이 아니라 JSON 하나**다 (`MatchIntentSchema`). 교체·6축과 토글 넷·
...
- **이 경기의 지난 턴 전부를 읽는다** — `<match_log>`(중계와 감독의 말, 턴당
  `MATCH_INTENT_TURN_CHARS`에서 자른다). 감독의 지시는 앞 턴의 대화를 잇는 말일 때가
...
| 이번 턴  | **이번 턴 인물지** + 감독 발화 + 상태 스냅샷(**감독 능력·평판·선수 명단 포함**)          | 감독 발화 + 장부 노트(패킷 없이) · 구간 대본·새 패킷은 `advance_match`의 **도구 결과**로 · 손잡이 턴은 대본을 이번 턴 층에 · 해석기에는 `<match_log>`(이 경기의 턴 전부), 마감에는 `<commentary>`·`<settlement>` | 매 턴     |
