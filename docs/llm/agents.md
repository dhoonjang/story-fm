...
| 전술 해석 (`tactic-orders.ts`)      | `tactic-orders`     | 두 GM의 `tactic_orders` 뒤 — 감독의 판 지시를 **구조화된 의도 하나**로 | 도구가 반려로 답한다 — 판은 그대로                     |
...
- **이 경기의 지난 턴 전부를 읽는다** — `<match_log>`(중계와 감독의 말, 턴당
  `TACTIC_TURN_CHARS`에서 자른다). 감독의 지시는 앞 턴의 대화를 잇는 말일 때가
