...
뒤에서 감독의 말을 JSON으로 옮기는 해석 넷(전술·훈련·시장·테이블), 경기의 판독기, 경기
마감, 사람이 읽지 않는 훈련 결산,
...
도구를 여럿 쥔 것은 GM 셋뿐이고, 나머지 아홉은 도구 없이 출력 스키마 하나로 답한다 —
...
| 테이블 해석 (`table-orders.ts`)    | `table-orders`      | 협상 GM의 `negotiation_orders` 뒤 — 방 안의 한 마디에서 값·조건·답·수락·철회를 **명령 인자**로                                                      | 도구가 반려로 답한다 — 장부는 그대로                                     |
...
`llm.yml`의 항목은 이 열과 **하나씩 맞는다**. 협상 상대를 연기하는 자리는 협상 GM 하나다 —
**감독이 직접 나선 라운드만** 모델이 답하고, 그 밖의 라운드는 코어가 앵커로 굳힌다(§4-1).
...
수락·흥정·지원)이다. 협상 상대에게 하는 말은 평시에 없다 — 감독이 직접 나서는 일이 곧
모드 진입이다(§4-1).
...
(§4-1), 협상 GM의 `negotiation_orders`도 원문을 코어가 넘긴다 — 방 밖의 사람에게 하는 말이
...
전부 같은 길이다** — 훈련 결산도, 스카우팅 평(§4-4)도, 이력 압축(§5-1)도, 그리고 경기
...
| 단계      | 무엇을                                                                            |
| --------- | --------------------------------------------------------------------------------- |
| **턴 앞** | 손잡이의 시간 이동 · 지난 턴이 받아 둔 카드 · 손잡이 턴의 전술판 적용             |
...
`pendingNegotiation`), 감독이 나서는 게이트를 지나면 방 안의 턴은 전부 협상 GM으로
...
| ③    | `negotiation_orders` 도구 | 감독의 말에 값·조건·답·수락·철회가 실렸을 때 한 번 — **테이블 해석**(LLM)이 명령 인자로 옮기고 코어가 건다 → 걸린 것·반려된 것과 새 앵커가 도구 결과로                                     |
| ④    | `counterparty_reply` 도구 | **상대의 답을 판정한다** — 들은 것(말투·논거) · 태도 · 오퍼가 올라 있으면 판정 · 부르는 조건. 코어가 사실 대조하고 인내를 깎고 앵커 ± 한도로 잘라 반영 → 장부 줄과 남은 인내가 도구 결과로 |
| ⑤    | `leave_negotiation` 도구  | 감독이 그만하겠다는 말을 했을 때 — 협상은 `open`으로 두고 방만 닫는다                                                                                                                      |
| ⑥    | **협상 GM**               | 도구 결과를 읽고 **장면과 상대의 대사**를 쓴다 — 자른 판정 위에 서므로 말과 장부가 어긋나지 않는다                                                                                         |

- **읽는 것** — `NEGOTIATION_GM_SYSTEM` · 레퍼런스(구단·감독 카드 · `<counterparty>`
  서류: 갈래·양쪽·`<voices>`(건너편 한 사람)·선수의 사실·인물지 · `<situation>`) · 이 방의 대화 전부(방의
...
- **부르는 것** — 감독이 값·조건·답을 말한 턴에 `negotiation_orders` 한 번(오퍼가 서고 앵커가
  다시 선다), 그 뒤 **같은 턴에** `counterparty_reply` 한 번 — 오퍼가 올라 있으면 이 답에서
  판정한다. 말만 오간 턴도 상대가 답할 것이 있으면 `counterparty_reply`다 — 말투와 논거가
  인내를 움직이는 자리는 그것뿐이다. 감독이 그만하겠다고 하면 `leave_negotiation`. 감독이
  나선 첫 턴(`opening`)은 도구가 없다 — 그 자리가 어디인지와 건너편 사람들, 상대의 첫
  말까지만 쓴다. 장면의 마지막 줄은 평시와 같은 제안 한 줄이다(§2).
- **감독의 말은 코어가 넘긴다.** `negotiation_orders`는 인자가 없다 — 이번 턴 감독의 발화
  원문이 해석기의 `@감독:` 줄이 된다(§1). 한 턴에 한 번이다.
- **매체는 협상 GM이 첫 턴에 정한다.** 그 자리가 사무실인지 전화인지 오간 편지인지는
  서류·상황·감독의 말에 이미 들어 있고, 장면 헤더의 장소와 첫 장면이 그것을 말한다 —
  그다음부터는 방의 이력이 잇는다. 고를 목록도 세이브의 칸도 없다.
- **상대의 대사는 협상 GM이 직접 쓴다.** 방이 읽는 것은 이 협상의 서류·상황·대화뿐이라
  ...
  — 다시 나선 자리가 뒤에 읽는 것은 그 사실이다.
- **방을 나오는 길은 셋이다.** 합의·결렬은 협상이 `open`을 벗어나는 순간 코어가 방을
  닫고(턴 뒤 — `closeNegotiation`), 물러나기는 `leave_negotiation` 도구 또는 화면의
  손잡이다. 단장의 수락이 이적료만 굳힌 자리(`feeAgreed`)는 합의가 아니다 —
...
- **방 안에서 날짜는 흐르지 않는다.** 장면 헤더는 그 날 안의 시각만 옮긴다(`applyScenePoint`
  — 판이 열린 국면과 같은 규칙). 시간을 넘기는 손잡이는 방 안에 없다.
...
논거도 오가지 않고, 논거는 오퍼를 넣는 순간 코어가 이미 판정했다(`evaluatePitch`).
남는 것은 앵커에서 사다리 한 칸·금액 ±15%·조건 부르기·대사뿐인데, 그 셋은 위임이 이미
포기하고 굳히는 것들이다(→ [../simulation/transfer.md](../simulation/transfer.md) §12-4).
...
- **감독이 답할 차례는 그다음이다** — 되부른 값은 협상 요약 줄(`describeNegotiations` —
  「조정 … 도착」)이 들고, 그 답은 감독의 뜻이라 평시 GM의 `market_orders`가 옮긴다.
...
코어가 굳히므로 두 모드가 같다. 방에서는 대본이 손잡이와 `counterparty_reply`를 부르고
(`negotiationScript`) 판정을 비우므로 코어가 앵커로 자른다. 실패한 실호출과 **같은 함수**를
지나므로 두 모드가 확률을 다른 사다리로 가르는 일이 없다 (§8). 테이블 해석기도 대본이 채운다 — 표에 있는 말만 명령이 되고, 없는 말은
그대로 상대에게 간다.
...
| 교섭 서류·상황 (협상 방)                | `packages/agents/src/counterparty-brief.ts` · `table-situation.ts`             |
