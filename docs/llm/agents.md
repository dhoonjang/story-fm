...
| 지시 해석 (`apply-orders.ts`)       | `apply-orders`      | 두 GM의 `apply_orders` 뒤 — 감독의 판 지시를 **구조화된 의도 하나**로 | 도구가 반려로 답한다 — 판은 그대로                     |
...
강제하는 그릇이지 스킬이 아니다.** 경기 중 고정층은 셋뿐이라 평시(49개)보다 훨씬
작다(§5).

**평시 GM도 전술의 파라미터를 직접 채우지 않는다.** 라인업·1·2군 이동·6축·개인 지시·
세트피스·지역 플랜·공략은 `apply_orders(orders)` 하나로 온다 — 감독의 말을 원문
그대로 넘기면 도구 뒤의 지시 해석이 `Orders`로 옮기고 코어가 스킬로 적용한다. 판을
세우는 일은 구조가 크고 낱말이 많아(자리·역할·갈래·세트피스 축) 장면을 쓰는 머리가
같은 왕복에서 채우면 하나를 빠뜨린다. 대화·훈련·이적·재정처럼 인자가 판정 하나거나
값 몇 개인 도구는 GM이 직접 채운다.
