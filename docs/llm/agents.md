...
**하는 일은 열한 갈래이고 설정 키는 열이다.** 평시의 GM과 부임 첫 장면, 경기의 매치 GM,
도구 뒤에서 감독의 말을 JSON으로 옮기는 해석 셋(전술·훈련·시장), 협상 상대, 경기 마감,
사람이 읽지 않는 훈련 결산, 밀려나는 이력을 요약으로 옮기는 압축, 그리고 새 게임의
배경을 한 번 읽는 온보딩 판정. 어느 쪽도 상태를 직접 바꾸지 못한다 — 도구 호출과 앵커 ± 한도만이 통로다.
...
GM 둘뿐이고, 나머지 여덟은 출력 스키마 하나로 답한다.
...
| 온보딩 판정 (`onboarding-judge.ts`) | `onboarding-judge`  | 새 게임의 배경 → **지갑 · 능력치의 결 · 시작 사건**                | 삼키고 **코어 앵커가 남는다** (§4-2)                   |

설정 이름이 곧 `config/llm.yml`의 키이자 계측 장부의 키다 ([models.md](./models.md)).
`llm.yml`의 항목은 이 열과 같다 — 첫 장면은 `gm`의 키를 쓴다. 협상 상대는 편지든
...
- **읽는 것은 `<anchor>`·`<club>`·`<background>` 셋**이다. 세이브는 휴리스틱 능력치로
  이미 서 있고(그것이 능력치의 앵커다) 이력은 없다. `<club>`은 시작 사건이 걸 수 있는
  사람의 목록이다 — 구단주·수석코치·주장·핵심 선수·유망주의 id.
...
- **`LLM_MODE=mock`은 이 호출을 하지 않는다** — 앵커가 그대로 지갑이다 (§4-1의 mock과
  같은 규약).
- **게임 도구는 없다.** 산출 도구 `report_onboarding` 하나로 셋을 낸다.
