...
**하는 일은 여덟 갈래이고 설정 키는 일곱이다.** 평시의 GM, 경기의 지시 해석과
캐스터, 협상 테이블 건너편에 앉는 상대, 사람이 읽지 않는 훈련 결산, 밀려나는 이력을
요약으로 옮기는 압축, 그리고 새 게임의 배경을 한 번 읽는 온보딩 판정. 어느 쪽도 상태를
직접 바꾸지 못한다 — 스킬 호출과 앵커 ± 한도만이 통로다.
...
없다: 경기 평점은 그 경기를 중계한 캐스터가 종료 턴에 매기고(§3), 심경은 그 선수와
있었던 일을 쓴 스킬이 한 문장을 함께 남긴다(§4-3).
...
| 매치 캐스터 (`match-caster.ts`)     | `match-caster`      | 확정된 사건을 중계·연출·벤치 대화로 · **종료 턴의 결산** | 오류 — 장부는 그대로 (결산은 앵커가 남는다)   |
...
`llm.yml`의 항목은 이 일곱과 같다 — 첫 장면은 `gm`의 키를 쓴다.
...
**게임 도구를 쥔 것은 GM뿐이다.** 경기의 두 에이전트는 경기를 **바꿀** 도구가 없다 —
해석은 산출 도구 하나(`report_intent`)로 JSON을 내고 캐스터는 문장만 쓴다. 산출
도구는 훈련 결산의 `report_training`과 같은 자리로, **구조화 출력을 강제하는 그릇이지
스킬이 아니다.** 캐스터가 종료 턴에 한 번 받는 `settle_match`(§3)도 같은 그릇이다 —
끝난 경기의 평점·적응도·능력치·심경을 앵커 ± 한도 안에서 내는 것이지 경기를 바꾸는
것이 아니다. 게임 도구 정의는 고정층이라 캐시 프리픽스를 부풀리는데(§5), 경기 중에는
그 값을 치를 이유가 없다.
...
- **산출은 스킬 호출이 아니라 JSON 하나**다 (`MatchIntentSchema`, Zod). 교체·6축과
  **토글 넷**(전환·오프사이드 트랩·태클 강도·GK 배급 —
  ...
  지시·지역 플랜·공략·**세트피스 키커와 인원**·대화·진행 여부가 한 객체의 필드로 함께 온다. 한 번에 다 보므로
...
- **직전 중계 턴을 읽는다.** 해석기의 입력은 장부와 이번 턴 한 줄이 아니라 그 앞에
  **이 경기의 마지막 중계 턴**(`MATCH_INTENT_CONTEXT_TURNS`)이 `<last_turns>`로 선다 —
  평시 GM의 「이름 없이 가리키면 직전 대화의 대상이다」와 같은 자리다. 직전 턴에 어떤
  선수를 두고 대화한 뒤 "걔 빼"가 `unresolved`로 떨어지던 자리이고, 이력 전체를 실으면
  분류기가 중계 스무 턴을 정가로 읽는다.
...
- **다시 불러도 자국이 겹치지 않는다.** 이 호출은 도구가 없어 판을 바꾸지 못하고, 판을
  ...
  그대로 무너진다(§8). 종료 턴은 결산 표식(`MATCH.result.rated`)이 하나 더 자국이다.

### 종료 턴 — 경기를 중계한 사람이 결산한다

③에서 구간이 `finished`로 굴러가고 승부차기도 남지 않은 턴이 **종료 턴**이다. 이 턴의
④는 다른 턴과 순서가 하나 다르다 — **`finalizeMatch`가 캐스터 호출보다 앞이다.**

| 걸음 | 무슨 일                                                                                                |
| ---- | ------------------------------------------------------------------------------------------------------ |
| ③′   | 구간이 끝났다 → 장부 노트·중계 이력·장부의 분·**평점 브리프**를 잡아 둔다 (장부가 살아 있을 때만 된다) |
| ③″   | `finalizeMatch` — 기준 평점(앵커)이 `MATCH.result.ratings`에 박히고, 마감 기록·소식이 선다             |
| ④    | 캐스터 호출 — **첫 왕복은 `settle_match`가 강제**되고(`toolChoice`), 그 뒤 마무리 중계를 쓴다          |

- **중계를 쓴 머리가 그 경기의 평점을 매긴다.** 사건 목록으로 캐스터가 이미 지어낸
  흐름을 다른 호출이 같은 목록으로 한 번 더 지어내던 자리다 — 중계 본문·하프타임
  팀토크·외침은 캐스터의 컨텍스트에 캐시된 채 있고, 평점은 그것을 못 보는 싼 호출이
  매겼다. 이제 그 컨텍스트가 평점·적응도·능력치·심경을 한 번에 낸다.
- **브리프 표는 이번 턴 층이다.** 출전 선수마다 자리·선발/교체·출전 시간·기준
  평점·나이·성장 여지·전술 적응·기록이 `<settlement>`로 감독 발화 뒤에 선다 — 종료 턴
  한 번뿐이라 고정층에 둘 이유가 없다. 무엇을 매기고 어디까지 벗어날 수 있는지는
  **도구 설명**이 갖는다 ([prompts.md](./prompts.md) §5-2).
...
- **경기를 바꾸는 도구는 여전히 없다.** `settle_match`는 끝난 경기의 결산이지 판이
  아니고, 그 하나뿐이다. 첫 왕복이 그것을 강제하므로 "그 도구로만 답하라"는 적지
  않는다 ([prompts.md](./prompts.md) §5-3).
- **결산이 실패해도 경기는 끝난다.** 도구가 안 불리거나 검사에 걸리면 앵커가 그대로
  남는다 — 마감은 이미 ③″에서 끝났다. 재시도 가드는 표식(`matchRated`)과 나간
  델타 둘이다.
...
| 경기 (`settle_match`)   | 종료 턴, `finalizeMatch` 뒤               | **캐스터** — 종료 턴 첫 왕복 (§3) | 출전 선수 전원                      | 앵커 ±1.2 + 근거 한 줄 · 전술 적응도 −2\~+8 · 능력치 11명까지 · 심경 8명까지    |

- **훈련만 별도 호출로 남는 이유는 무게다.** 구간의 훈련 표 25행은 장면 턴에 얹기엔
  무겁고, 산출 25행이 장면을 밀어낸다 — 싼 자리가 맞다. 경기 결산은 그 반대다: 읽어야
  할 것(중계·라커룸)이 이미 캐스터의 컨텍스트에 있다.
...
| `settle_match` (캐스터 종료 턴, §3) | `moods` (≤ `MOOD_BATCH`) | 출전 선수                            |
...
| 이번 턴  | **이번 턴 인물지** + 감독 발화 + 상태 스냅샷(**감독 능력·평판·선수 명단 포함**)          | 감독 발화 + 구간 대본 + 경기 장부 노트 · 종료 턴에만 `<settlement>`와 `settle_match` 도구 하나(§3) · 해석기에는 직전 중계 턴 `<last_turns>` | 매 턴     |
...
- **도구를 부르지 않은 응답도 실패다.** 산출 도구를 강제로 실은 호출
  ...
  턴이 취소되고, 결산은 두 번 실패하면 앵커가 남되 **로그는 남는다**. 종료 턴의 캐스터는
  다르다 — 도구가 안 불려도 중계가 왔으면 그 턴은 성공이고 평점은 앵커로 남는다(§3).
...
| 중계 프롬프트·구간 대본·경기 결산 도구 | `packages/agents/src/match-caster.ts`                                     |
