...
- **직전 중계 턴을 읽는다** — `<last_turns>`(`MATCH_INTENT_CONTEXT_TURNS`). 이름 없는
  지목이 직전 턴의 대상으로 풀린다.
...
| 이번 턴  | **이번 턴 인물지** + 감독 발화 + 상태 스냅샷(**감독 능력·평판·선수 명단 포함**)          | 감독 발화 + 장부 노트(패킷 없이) · 구간 대본·새 패킷은 `advance_match`의 **도구 결과**로 · 손잡이 턴은 대본을 이번 턴 층에 · 해석기에는 `<last_turns>`, 마감에는 `<commentary>`·`<settlement>` | 매 턴     |
