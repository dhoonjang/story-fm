...
| 지시 해석 (`match-intent.ts`)       | `match-intent`      | 두 GM의 도구 뒤 — 감독의 전술 지시를 **구조화된 의도 하나**로      | 도구가 반려로 답한다 — 판은 그대로                     |
...
세트피스·지역 플랜·공략은 `apply_tactics(orders)` 하나로 온다 — 감독의 말을 원문
그대로 넘기면 도구 뒤의 지시 해석이 `MatchIntent`로 옮기고 코어가 스킬로 적용한다. 판을
...
| 이번 턴  | **이번 턴 인물지** + 감독 발화 + 상태 스냅샷(**감독 능력·평판·선수 명단 포함**)          | 감독 발화 + 장부 노트(패킷 없이) · 구간 대본·새 패킷은 `advance_match`의 **도구 결과**로 · 손잡이 턴은 대본을 이번 턴 층에 · 해석기에는 `<match_log>`(이 경기의 턴 전부 · 평시엔 스쿼드 뷰와 지난 두 턴), 마감에는 `<commentary>`·`<settlement>` | 매 턴     |
