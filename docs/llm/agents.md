...
**하는 일은 여덟 갈래이고 설정 키는 일곱이다.** 평시의 GM, 경기의 매치 GM, 두 GM의
도구 뒤에서 감독의 전술 지시를 JSON으로 옮기는 지시 해석, 경기 마감, 사람이 읽지 않는
훈련 결산, 밀려나는 이력을 요약으로 옮기는 압축, 그리고 새 게임의 배경을 한 번 읽는
온보딩 판정. 어느 쪽도 상태를 직접 바꾸지 못한다 — 도구 호출과 앵커 ± 한도만이 통로다.
...
GM 둘뿐이고, 나머지 일곱은 출력 스키마 하나로 답한다.
...
그 뒤의 두 에이전트가 내는 것은 **출력 스키마** 하나뿐이다 — 해석의 `report_intent`,
...
- **산출 도구는 요청이 강제한다.** `report_intent`는 `toolChoice`로 실린다
