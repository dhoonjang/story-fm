...
```
<voices>
club "아스날" — 이적료 · 분할 연수 · 기한 · 파는 구단 관문 62%
agent "조르제 멘데스" — 주급 · 계약 지위 · 등번호 · 선수 관문 48%
</voices>
```
...
`LLM_MODE=mock`은 **앵커가 그대로 판정**이다. 규칙 기반 mock GM도 같은 앵커를 읽으므로,
두 모드가 확률을 다른 사다리로 가르는 일이 없다 (§8). mock GM은 테이블에 앉지 않는다.
...
- **`LLM_MODE=mock`은 이 호출을 하지 않는다** — 앵커가 그대로 지갑이고 첫 장면은
  규칙 장면(`buildOnboardingTurn`)이다 (§4-1의 mock과 같은 규약).
...
엔진 명령 경로만 사용**하고, 목적은 서사 품질이 아니라 "시나리오가 끝까지 도는가"다.
결산(훈련·경기)은 아예 부르지 않아 코어 앵커가 그대로 남는다 — 그래서 mock으로 잰
