...
| ③    | `apply_orders` 도구   | 감독의 지시를 **지시 해석**(LLM)이 `MatchIntent`로 옮기고 코어가 스킬로 적용 — 시계는 그대로                  |
| ④    | `advance_match` 도구  | ③과 같되 그 뒤 **한 구간을 굴린다** → 구간 대본 + 구간 뒤 장부·패킷이 도구 결과로                             |
...
- **읽는 것** — `MATCH_GM_SYSTEM` · 레퍼런스(구단·감독·수석코치 카드·`<pre_match>`) ·
  ...
  새 패킷**이 함께 온다.
- **부르는 것** — 진행하라는 말(「계속」·「봅시다」·교체하며 이어서 보자)에만
  `advance_match`; 지시만 있고 진행이 아니면 `apply_orders`; 이름을 부르거나 말만 건
  턴은 도구 없이. 장부가 `finished`면 `finalize_match`. 규칙은 도구 설명이 갖는다
