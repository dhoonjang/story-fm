...
`@감독: <원문>`, 도구 하나를 `toolChoice`로 강제. `<recent_turns>`·`<match_log>`는 이번
턴에 밀어 넣은 꼬리를 빼고 선다(`pastTurns`) — 감독의 말은 `@감독:` 줄 하나로만 선다.
셋이 한 턴에 함께 불리면 같은 원문을 받되 각자 제 영역만 옮기고, 남의 영역인 말은
`unresolved`에 남기지 않는다.
