...
- ⚠️ **주급 예산은 이 축을 쓰지 않는다** — 연 매출에서 **고정비를 뺀 뒤** 나오는 값이다
  ...
  보는 천장은 다섯 시즌 내내 상수**라 번 돈이 나갈 축이 없었다. 체급이 주급에 닿는
  길은 이제 체급 → 구장·브랜드 → 매출 → 천장이다.
...
U21을 붙이는 만큼 1군은 얼마든지 불어난다 — 실제로 말라가가 등록 25 + U21 7명으로
서른둘이 됐다. 규정이 안 막는다고 서른을 넘겨 데리고 다니는 1군은 없어서
...
- ⚠️ **같은 기준에는 지정 선발 가산도 들어간다** (`memoFit(preferred)` 하나를
  ...
  세울 수 있는 모양인데도 미달자가 나와, 아스날·리버풀·토트넘이 이유 없이 리서치
  모양을 버렸다.
...
받는데(§5), 승격을 먼저 적용하고 배치를 나중에 검증하던 때는 반려된 요청이 승격만
남겼다 — GM 경로는 턴이 끝날 때 저장하므로 "반려했습니다"를 읽은 감독의 스쿼드가
이미 달라져 있었다. 순서는 **전부 검증 → 승격 → 배치 → 강등**이고, 하나라도 걸리면
아무것도 바꾸지 않는다.
