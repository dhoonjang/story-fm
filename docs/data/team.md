...
TeamCatalogEntry {
  ...
  honours?,              // 게임이 시작되기 **전의** 우승 — [{ competitionId, count, lastYear? }]
}
```
...
**`honours` — 세계에 역사가 있게 하는 시드.** 게임은 t=0에서 시작하므로 장부에는
...
`TROPHY`를 더한 것이고(`clubRecordsOf` — [../simulation/career.md](../simulation/career.md) §6),
시드는 t=0의 초기치라 진행 중인 세이브는 이 표를 다시 읽지 않는다.
...
- **검증 범위** — 전수가 아니다. 다섯 리그의 잘 알려진 클럽만 시드를 갖고, 나머지는
  ...
  숫자가 틀린 것이 없는 것보다 나쁘다. **`lastYear`는 시드가 들지 않는다**(optional) —
  마지막 연도까지 확인된 대회가 드물고, 게임이 시작된 뒤의 마지막 우승은 `TROPHY`가
  시즌으로 답한다. 어드민이 확인된 연도를 채울 자리로 남긴다.
- 리그 id는 **그 나라 1부의 통산 우승**이다 — 프리미어리그 출범 전의 잉글랜드 1부
  우승도 `epl`에 든다. 분데스리가만 1963년 출범 이후만 센다(그 전은 리그가 아니었다).
- 어드민에서 편집한다(`.data/team-catalog.json`) — 나머지 카탈로그 필드와 같은 길.
...
| 어디                                       | tier 1 / 2 / 3 / 4                                       |
| ------------------------------------------ | -------------------------------------------------------- |
| 절차 생성 능력치 기준선 (`TIER_BASE`)      | 84 / 80 / 76 / 72 — 실선수가 없는 스쿼드(§4) · 유스 천장 |
...
| 최근 성적   | 최근 3시즌 리그 최종 순위(`state.history` · `recentForm` 파생)의 리그 크기 대비 백분위 | 0.20   |
...
- ⚠️ **성적 축의 원본은 전 클럽이 같다.** 시즌 롤오버가 그해 리그전을 돈 리그마다 최종
  순위표를 통째로 남기고(`recordSeasonHistory` → `state.history` — 승강을 적용하기 **전에**;
  체급 재산정은 그중 최근 3시즌만 읽는다), 세 축이 그 표 하나를 읽는다. 감독의 `SEASON_RECORD`는 커리어 기록이지 이
...
| 무엇      | 어디서                                  | 왜                                                                                    |
| --------- | --------------------------------------- | ------------------------------------------------------------------------------------- |
| 색 두 벌  | `colours.primary` · `colours.secondary` | 구단색 두 가지, 그 순서 — 실제 구단의 공식 값. 없으면(어드민이 만든 클럽) id 해시     |
| 유채 강조 | `colours.accent`                        | 흰·검 구단의 유채색 — 리그 색을 세는 신호이고, 앞 두 색이 다 어두울 때의 띠 후보      |
| 띠        | `colours.band`                          | 카탈로그가 리그 안에서 서로 갈리게 재어 붙인 파생값 — 공식 값이 아니고 색 표에도 없다 |
...
- **색은 구단이 스스로 드는 구단색 두 가지, 그 순서다.** 킷은 해마다 바뀌어도 이 순서는
  남는다 — 쾰른은 흰 셔츠를 입어도 빨강·흰이고, 레알 마드리드·풀럼은 흰, 유벤투스는 검이
  대표색이다. 유채색을 앞세우려고 순서를 바꾸지 않는다. 실제로 같은 구성인 구단(유벤투스·
  우디네세의 흑백, 풀럼·발렌시아의 흰·검, 붉은 구단 여럿)은 현실에서 그렇듯 문장의
  모양·글자·강조색으로 갈린다. 세 값을 전부 같이 갖는 구단은 둘이 없다 — `crest.test.ts`가
  잰다.
...
- **등록 현황은 저장하지 않고 1군 명단에서 파생한다**(`squadRegistrationOf` →
  `listed`·`homegrown`·`under21`·`goalkeepers`·`openHomegrown`·`openNonHomegrown`·`issues`).
  GM 컨텍스트·조회 도구에는 `registrationLine`이 한 줄로 싣는다.
- **1군에 골키퍼가 없으면 명단이 성립하지 않는다** — `issues`에 「골키퍼 부족」이
  선다. 골문은 필드 선수가 대신 설 수 없는 유일한 자리라, 필드 자리를 세지 않고
  이 하나만 센다. 최소가 1인 이유도 그것이다 — 벤치의 두 번째 골키퍼는 운영의
  선호이지 명단이 서는 조건이 아니다. 재계약을 놓쳐 골키퍼를 잃는 것은 감독의
  ...
  (→ [../simulation/match.md](../simulation/match.md) §2). AI 구단에는 그 경고를
...
- **명단이 찬 채로 영입하면 그 선수는 2군으로 들어온다.** 계약은 성립하고 등록만
  ...
  완장(주장·부주장)에서 함께 빠진다 — 라커룸 서열의 후보도 1군뿐이다
  (→ [people.md](./people.md) §5-1).
...
- **전력이 아니라 뎁스다.** 기준선은 그 클럽 **주전 열한 명 평균에서 네 칸 아래**다.
  ...
  첫 시즌부터 중위권이 된다. 클럽 자신의 명단에서 파생하므로 체급은 저절로 따라온다.
  **무리로 보면 주전 아래에 선다** — 다섯의 평균이 주전 평균 아래다. 그 위에 축
  표집이 얹히므로 잘 뽑힌 하나가 주전에 끼는 일은 있고, 그것은 규칙이 깨진 것이
  아니라 뽑기다.
...
### 초기 토글 넷 — 프리셋이 자기 스타일이 말하는 갈래에만 선다

6축과 같은 함수(`initialTactics`)가 전환·트랩·태클·GK 배급의 갈래
([match.md §1.2](../simulation/match.md))도 함께 세운다.

| 스타일       | 팀 수 | `transition` | `offsideTrap` | `tackling` | `keeperDistribution` |
| ------------ | ----- | ------------ | ------------- | ---------- | -------------------- |
| `high-press` | 26    | `counter`    | 켬            | `hard`     | —                    |
| `possession` | 24    | `regroup`    | 켬            | `soft`     | `short`              |
| `direct`     | 15    | `regroup`    | —             | —          | —                    |
| `transition` | 13    | `counter`    | —             | —          | `long`               |
| `low-block`  | 10    | `regroup`    | —             | `hard`     | `long`               |
| `balanced`   | 8     | —            | —             | —          | —                    |

**빈칸은 중립이고 중립은 델타가 0이다** — 프리셋은 자기 스타일이 **실제로 말하는**
갈래에만 선다. 비어 있는 자리는 셋 다 "같은 사실을 두 번 싣지 않기 위해"다:
`direct`의 롱볼은 이미 `passStyle` 5가 말하고, `high-press`는 압박으로 이미 상대
진영에서 공을 얻으므로 배급까지 짧게 묶을 이유가 없다. `low-block`이 `counter`가
아니라 `regroup`인 것도 같은 규칙이다 — 역습으로 사는 팀은 `transition` 프리셋이
맡고, 로우블록은 뒤에서 버티는 쪽이다.

⚠️ **표는 리그 합을 보고 짠다 — 스타일이 아니라 팀 수로 가중해서.** 6축이 리그 평균
3에 서야 하는 것과 같은 이유다([match.md §1.2](../simulation/match.md)): 갈래가
한쪽으로 쏠리면 리그 전체가 같은 방향의 이득과 대가를 달고 선다. 스타일 여섯을 같은
무게로 놓고 맞추면 안 된다 — 위 표의 팀 수가 26에서 8까지 갈린다. 지금 표의 96팀
가중 합은 **공격 +0.29% · 중원 +0.11% · 수비 −0.02% · 강도 +1.0%**다. 프리셋이나
시드 스타일을 고칠 때는 이 합을 다시 재라.
...
- **배정은 슬롯 전체 합의 전역 최댓값이다** (`fillSlots` — 직사각형 Hungarian).
  ...
  끌어가 뒷자리를 무너뜨린다.
- **골문에는 골키퍼만 앉는다.** 교차 감점(−400)은 "누가 덜 나쁜가"를 가르는 값이지
  "서도 되는가"의 답이 아니다 — 풀에 골키퍼가 하나도 없으면 감점을 안은 채로 수비형
  ...
  **골문을 비운 채 열 명**을 세운다. 빈 자리는 저장되지 않는 사실이라(배치는 선수
  ...
  킥오프까지 그대로면 자동 대체가 채운다(→ [../simulation/match.md](../simulation/match.md) §2).
- 새 게임의 전술 적응도는 전 팀이 `FAMILIARITY_BASELINE`(60)에서 출발한다.

### 빈 자리에 사람을 넣는다 (`openSeats`)

**자리가 먼저고 사람이 나중이다.** 감독이 좌표도 자리 코드도 말하지 않은 선발은 코어가
자리를 골라 세운다. 그 자리를 **선수의 주 포지션**에서 거꾸로 만들던 때는, 왼쪽 윙어가
계약 만료로 떠난 여름에 남은 오른쪽 자원 둘이 나란히 `RW`의 기본 좌표에 서고 왼쪽 측면이
아무도 없이 남았다 — 중원은 한 명, 이름 붙일 수 있는 모양이 아니다.

자리는 두 곳에서 온다.

1. **이번에 선발에서 빠진 사람이 비운 자리** — 교체는 자리를 잇는다. 감독이 만들어 둔
   판의 모양이 사람만 바뀐 채 그대로 남는다.
2. 그것으로 모자라면 **포메이션의 빈 자리**(`openSeats` — `packages/domain/src/tactics.ts`).
   프리셋 열한 자리에서 지금 누가 서 있는 자리를 걷어내고 남은 자리다. 짝짓기는 **가까운
   쌍부터 전역으로** 짓는다(거리 오름차순, 같으면 선 순서·자리 순서) — 앞에서부터 각자
   제일 가까운 자리를 집게 하면 먼저 선 사람이 뒤 사람의 유일한 자리를 가져가, 멀쩡한
   배치에서도 엉뚱한 자리가 비는 것으로 읽힌다.

누가 어느 자리에 서는지는 **그 자리의 포지션 적응도**가 정한다 — 킥오프의 자동 대체와 같은
자다(→ [../simulation/match.md](../simulation/match.md) §2). 같으면 자리 순서, 그다음 id
사전순이라 같은 지시가 언제나 같은 판을 만든다. 열한 자리가 이미 다 찼을 때만 예전처럼 주
포지션에 세운다.

같은 규칙을 **시즌 전환의 배치 재구성**도 쓴다(→ [../simulation/season.md](../simulation/season.md)
§시즌 전환). 저장된 배치가 열한 칸에 못 미치면 모자라는 만큼을 포메이션의 빈 자리로 채우고,
그 자리에 새 스쿼드를 `buildAssignments`가 앉힌다.

**보정의 자리는 둘이고 하는 일이 다르다.**

| 언제                    | 무엇                        | 어디                                                              |
| ----------------------- | --------------------------- | ----------------------------------------------------------------- |
| 배치가 저장될 때        | **빈 자리에 사람을 앉힌다** | `commands/lineup.ts` · `competition/season.ts`                    |
| 킥오프 직전, 패킷에서만 | **무너진 줄의 좌우를 편다** | `sim/lineup-cover.ts` (→ [match.md](../simulation/match.md) §1.7) |

앞의 것은 **자리를 고르고**, 뒤의 것은 이미 선 자리의 x를 줄 안에서 다시 나눌 뿐이다. 둘이
겹치지 않는 이유가 여기 있다 — 자리를 만드는 쪽은 저장하고, 줄을 펴는 쪽은 패킷에만 있다.
그래서 **이미 저장된 판은 코어가 펴서 다시 쓰지 않는다**: 한쪽으로 사람을 모으는 것은 이
게임이 지원하는 수이고, 감독이 둔 판을 코어가 고쳐 저장하면 그가 만들지 않은 배치가 다음에
판을 열 때 서 있다. 예전 규칙으로 이미 저장된 판도 마찬가지로 그대로 남고, 감독이 판을
한 번 만지거나 다음 자동 채움이 그 자리를 다시 고를 때 제 모양으로 돌아온다.

**벤치는 자리를 갖지 않는다.** 좌표는 선발만의 것이고(`setLineup`), 패킷도 벤치 슬롯에
좌표를 싣지 않는다. 벤치 명단을 선발과 **같은 문**(`slotsFor`)으로 보내면 열한 자리가 통째로
「주인이 떠난 자리」로 보여 벤치 아홉이 적응도 순으로 선발의 좌표에 그대로 앉는다 — 명단
화면이 한 점에 두 사람을 그리던 자리다.
...
**정원이 차면 화면의 손잡이가 잠긴다.** 전술판 명단의 「매치데이 벤치로」는 벤치가 아홉이
된 순간 눌리지 않고, 계기판(`벤치 9/9`)이 글자 1층으로 올라서 그 이유를 대신 말한다 —
안내 문구는 두지 않는다([../ui/design-system.md](../ui/design-system.md) §1 조작). 넘치는
지정을 화면이 조용히 삼키면 저장은 끝났다고 뜨는데 선수는 벤치에 없고, 감독은 넣었다고
믿은 채 킥오프를 맞는다. **계기판이 세는 몫과 잠금을 여는 몫은 같은 하나다** — 갈리면
8/9인데 잠긴 손잡이가 생기고, 그때 화면이 무엇을 세고 있는지는 아무도 알 수 없다.
벤치에서 **빼는** 길은 정원과 무관하게 늘 열려 있다.
...
- **자리 표기는 선발과 벤치 양쪽 다 읽고, 표기를 너그럽게 받는다.** 코어의 코드는
  좌표가 파생되는 이름이라 한 자리에 하나뿐인데(`POSITION_ANCHORS`) 감독과 GM은
  `AML`·`DC`·`STC`처럼 자기 표기로 부른다. 별칭표 하나(`POSITION_ALIASES` ·
  `normalizePositionCode` — `packages/domain/src/tactics.ts`)가 그것을 코드로 옮긴다.
  **그래도 읽지 못한 표기는 그 자리 하나만 버린다** — 그 선수는 원래 자리(선발이었으면
  지금 좌표, 아니면 주 포지션)에 서고, 읽지 못한 표기가 결과에 적힌다. 통째로 반려하면
  열한 명을 이름과 자리까지 정확히 부른 지시가 표기 하나 때문에 사라진다.
- **선발은 열한 명을 다 부르지 않아도 된다.** `starting`이 1~11이고, 부른 자리만
  바뀌고 **남은 자리는 지금 선발이 지금 좌표로 지킨다**(`completeStarting`). 평시에
  벤치 선수를 그라운드에 세우는 문은 이 명령 하나뿐이라(`set_player_tactic`은 이미 뛰는
  선수의 자리만 옮긴다) 열한 명을 다 적어야만 걸리면 "골문에 킬브라이드" 같은 지시는
  걸릴 길이 없다.
  - **자리를 잃는 사람은 감독이 부른 자리에 가장 가까이 서 있던 사람이다.** 코어가 셀
    수 있는 사실은 자리의 거리뿐이다 — 누가 더 잘하는지, 누가 로테이션 차례인지는
    감독의 판단이고 코어가 대신 고르면 감독이 내리지 않은 결정이 판에 오른다. 거리가
    같으면 id 사전순이라 같은 지시가 언제나 같은 판을 만든다.
  - **밀려난 선발은 벤치 맨 앞에 선다** — 방금 자리를 뺀 선수는 교체 카드로 쓸 사람
    이고, 정원(9)을 넘치는 만큼은 벤치 끝에서 예비로 내려가며 결과가 그 이름을 적는다.
    `bench`를 명시하면 그 지정이 이긴다.
  - **지금 선발이 열한 자리를 채우지 못하면 반려한다** — 몇 명이 비었는지 적어서.
    코어가 벤치에서 골라 채우면 그것도 감독이 하지 않은 결정이다.
- 2군으로 내려간 선수가 전술 배치에서 빠지는 것은 §5 그대로다. 다만 **선발이던
  선수를 내리면 결과가 그것을 적는다** — 판이 열 명이 된 것은 감독이 결과로 알아야
  할 사실이지 조용히 넘길 일이 아니다.

**세트피스 키커도 같은 요청에 실린다.** 전술판이 보내는 `setPieceTakers`는 **서버와
달라진 자리만** 담고(역할과 같은 규약), 라우트는 그것을 `set_set_piece_takers`에
그대로 넘긴다 — 값의 검증은 명령 하나가 갖는다. **가담·수비 두 축(`setPieceRoutine`)도
같은 자리, 같은 규약**이다 — 달라진 축만 담고, 옮기는 문은 `setSetPieceRoutine` 하나다
(→ [match.md](../simulation/match.md) §1.4). 같은 지정을 매 자동 저장마다 다시
보내면 바뀐 것이 없는데도 편집 노트가 남으므로, 무엇이 달라졌는지는 **화면이** 서버
값과 견줘 가른다. 반려는 배치를 되돌리지 않는다(역할과 같은 자리).

**명단이 이름 앞에 세우는 것은 표식 셋이 아니라 세 칸짜리 표식 하나다.** 코너·프리킥·
페널티가 늘 같은 순서로 서고 맡지 않은 자리는 빈 칸으로 남으므로, 한 선수가 셋을 다
차도 이름이 시작하는 자리가 움직이지 않는다 — 표식 셋이 각자 테두리를 갖던 때는 그
한 행만 이름 폭의 4분의 1을 표식에 내주었다.

세 갈래는 **글자의 모양**이 가른다. 색과 툴팁 문장에만 뜻을 두면 감독은 표식을
누르거나 오래 물고 있어야 그것이 무엇인지 알게 된다.

| 그 자리                             | 모양           |
| ----------------------------------- | -------------- |
| 감독이 지정했고 지금 찬다           | 진한 금색 글자 |
| 지정이 없어 코어가 세웠다           | 옅은 회색 글자 |
...
[match.md](../simulation/match.md) §1.4·§2.
...
- **컵은 축소 세계에서 열리지 않는다.** 국내 컵 32강·대항전 24팀은 다섯 리그 전체를
  전제하므로 여덟 팀짜리 세계에서 성립하지 않는다(`hasCups`). 2부 클럽도 컵이
  있을 때만 존재한다 — 국내 컵 인원이 그 클럽들의 자리다.
