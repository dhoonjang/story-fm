...
| 무엇      | 어디서                                  | 왜                                                                                    |
| --------- | --------------------------------------- | ------------------------------------------------------------------------------------- |
| 색 두 벌  | `colours.primary` · `colours.secondary` | 실제 구단의 공식 색 — `primary`는 팀마다 유일하다. 없으면(어드민이 만든 클럽) id 해시 |
| 유채 강조 | `colours.accent`                        | 어두운 화면의 가는 자리용 — 흰·검 킷 구단은 두 번째 공식 유채색                       |
...
- **대표색은 팀마다 다르다.** 열 팀이 같은 흰 방패를 들면 색은 「누구인가」에 답하지
  못한다. 킷의 첫 색이 흰·검인 구단(레알 마드리드·토트넘·우디네세 …)은 두 번째 공식
  유채색이 `primary`에 서고 흰·검은 `secondary`로 내려간다 — 값은 여전히 그 구단의 공식
  색이다. 유채색이 없는 구단(유벤투스)만 무채색이 대표색이다. `crest.test.ts`가 잰다.
