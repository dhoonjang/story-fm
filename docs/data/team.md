...
- 이름·약칭·리그·체급은 **필수**다 — 모든 클럽이 게임 시작에 갖는다.
...
- **가용(부상·정지·소집)을 보지 않는다.** 3주 뒤 경기의 결장자는 아무도 모르고, 오늘의
  ...
  상대 분석이 답한다 ([../simulation/match.md](../simulation/match.md) §3.6).
...
| 어디                           | 무엇                                                           | 문서                                     |
| ------------------------------ | -------------------------------------------------------------- | ---------------------------------------- |
| 경기 전 리포트 · 킥오프 게이트 | 더비 여부와 `heat` — 사실로 선다, 중계·화면이 읽는다           | [match §3.6](../simulation/match.md)     |
| 경기 강도                      | `1 + DERBY_INTENSITY_STEP(0.06) × heat` — 양 팀 카드·부상·파울 | [match §4.1](../simulation/match.md)     |
...
- **1군에 골키퍼가 없으면 명단이 성립하지 않는다** — `issues`에 「골키퍼 부족」이
  ...
  (→ [../simulation/match.md](../simulation/match.md) §3.1). AI 구단에는 그 경고를
...
([live-match.md §6.1](../simulation/live-match.md))도 함께 세운다.
...
3에 서야 하는 것과 같은 이유다([live-match.md §6.1](../simulation/live-match.md)): 갈래가
...
- ⚠️ **새 프리셋의 초기 6축은 리그 평균을 움직인다**(`initialTactics`). 리서치 성향이
  ...
  (`tactics-presets.test.ts`가 잡는다), 남은 여유는 축마다 한 칸의 절반이 못 된다.
...
- **골문에는 골키퍼만 앉는다.** 교차 감점(−400)은 "누가 덜 나쁜가"를 가르는 값이지
  ...
  **골문을 비운 채 열 명**을 세운다. 빈 자리는 저장되지 않는 사실이라(배치는 선수
  ...
  킥오프까지 그대로면 자동 대체가 채운다(→ [../simulation/match.md](../simulation/match.md) §3.1).
...
자다(→ [../simulation/match.md](../simulation/match.md) §3.1). 같으면 자리 순서, 그다음 id
...
**보정은 저장할 때 한 번뿐이다** — 빈 자리에 사람을 앉히는 것(`commands/lineup.ts` ·
`competition/season.ts`)이고, 이미 선 자리의 좌표는 건드리지 않는다. 그래서 **이미 저장된 판은
코어가 펴서 다시 쓰지 않는다**: 한쪽으로 사람을 모으는 것은 이 게임이 지원하는 수이고, 감독이
둔 판을 코어가 고쳐 저장하면 그가 만들지 않은 배치가 다음에 판을 열 때 서 있다.

**벤치는 자리를 갖지 않는다.** 좌표는 선발만의 것이고(`setLineup`), 경기도 벤치 슬롯에
...
(→ [match.md](../simulation/match.md) §3.5). 같은 지정을 매 자동 저장마다 다시
...
[match.md](../simulation/match.md) §3.5.
...
- **합성 보충은 그 클럽 실선수 최고를 넘지 않는다** (§4). 넘으면 세계 상위 명단에
  가명이 서고, 선발·팀 평점·이적 시세가 그 가명을 축으로 돈다.
