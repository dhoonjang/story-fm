...
| 스타일       | `transition` | `offsideTrap` | `tackling` | `keeperDistribution` |
| ------------ | ------------ | ------------- | ---------- | -------------------- |
| `possession` | `regroup`    | 켬            | `soft`     | `short`              |
| `high-press` | `counter`    | 켬            | `hard`     | `short`              |
| `transition` | `counter`    | —             | —          | —                    |
| `direct`     | —            | —             | —          | —                    |
| `low-block`  | `regroup`    | —             | `hard`     | `long`               |
| `balanced`   | —            | —             | —          | —                    |

**빈칸은 중립이고 중립은 델타가 0이다** — 프리셋은 자기 스타일이 실제로 말하는
갈래에만 선다. `direct`가 비어 있는 이유가 그것이다: 롱볼은 이미 `passStyle` 5가
말하고 있어 GK 배급까지 길게 두면 **같은 사실을 두 번 싣는다**. 같은 이유로
`transition`(역습)도 GK 배급을 비운다.

⚠️ **표는 리그 합을 보고 짠다.** 6축이 리그 평균 3에 서야 하는 것과 같은 이유다
([match.md §1.2](../simulation/match.md)) — 갈래가 한쪽으로 쏠리면 리그 전체가 같은
방향의 이득과 대가를 달고 선다. 지금 표의 96팀 합은 공격 +0.4% · 중원 −0.6% ·
수비 +0.2% · 강도 0이다(`hard`와 `soft`가 하나씩이라 카드 총량이 움직이지 않는다).
프리셋을 고칠 때는 이 합을 다시 재라.
