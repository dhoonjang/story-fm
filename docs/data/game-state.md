...
| 예 | 아스날의 이름·리그·초기 체급, 사카의 초기 15축 | 이 세이브의 아스날 체급, 사카 폼·계약·부상 이력 |
...
**게임 안에서 변하는 값은 세이브가 갖는다.** 구단 체급(`tier`)이 그 둘째 예다 —
카탈로그 값은 게임 시작의 초기치일 뿐이고, 그 뒤로는 `GAME_TEAM.tier`가 단일
소스이며 시즌마다 다시 매겨진다([team.md](team.md) §2). 체급을 읽는 자리는 전부
`tierOfTeamIn(state, teamId)` 하나를 지난다 — 카탈로그를 직접 읽으면 어드민이 편집한
순간 진행 중인 세이브의 보드 기대치와 경질 위험선이 그 자리에서 달라진다.
