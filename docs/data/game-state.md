...
| `history` `SeasonHistory`            | **시즌 결산 스냅샷** — 지나간 시즌마다 한 행. 리그별 최종 순위표(행 전체) + 그 시즌 감독 팀의 경기 결과                           | `domain/records.ts`     |
...
⚠️ **시즌이 넘어가도 지난 시즌은 남는다 — 다만 경기가 아니라 표로 남는다.**
`state.matches`는 새 시즌 일정으로 통째로 교체되므로, 그 전에 `state.history`가 그
시즌의 **결산 스냅샷** 한 행을 남긴다: 그해 리그전을 돈 리그마다 최종 순위표를 **행
전체**로(팀 id만이 아니라 경기·승무패·득실·승점), 그리고 **감독 팀의 경기 결과만**
(대회·단계·상대·홈/원정/중립·스코어·승부차기·날짜). 세계 전체의 경기 2,300건을
시즌마다 남기지 않는 이유는 값이다 — 전 경기는 시즌당 수 MB이고 결산 스냅샷은 **잰
값으로 시즌당 18KB**다(리그 5개·표 96행 11.4KB + 우리 경기 58행 6.8KB). 전 구단 우승을
드는 `trophies`가 시즌당 1.6KB를 더해 **시즌당 20KB**이고, 스무 시즌을 돌아도 0.4MB다 —
같은 세이브의 선수·기록 테이블이 이미 시즌당 3MB씩 는다.
남의 팀끼리의 맞대결 스코어는 **그 시즌이 지나면 사라진다**. 남는 것은 표와 우리
경기이며, 그 둘로 답할 수 없는 물음에는 답이 없다고 말한다.

우승자는 여기 따로 적지 않는다 — 리그 우승은 그 표의 1위이고, 녹아웃 대회의 우승·
준우승은 `TROPHY`가 **모든 구단**의 것으로 든다
(→ [career](../simulation/career.md) §6).
...
| 엔티티                                | 무엇                                                                                                                                                                                                                                                                                               | 정의                   |
| ------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------- |
| `negotiations` `Negotiation`          | 진행 중 협상 — 영입·매각·재계약·임대(양방향) · 조건서(`terms`) · 개인 조건 합의(`personal`) · 이적료 합의(`feeAgreed`) · 자리마다의 테이블(`tables.club`·`tables.agent` — 인내·줄) · 조항 발동(`buyout`) · 위임(`mandate` — 없음은 방침대로, 값은 단장이, `null`은 감독이 직접, transfer.md §12-4) | `domain/records.ts`    |
...
| `delegations` `Delegation`            | **위임 방침** — 갈래째 단장에게 맡긴 일. 갈래마다 한 줄이고 감독이 한 말이라 파생할 수 없다 (transfer.md §12-4)                                                                                                                                                                                    | `domain/records.ts`    |
...
**`SAVE_VERSION = 8`** (`core/persistence.ts`). 버전이 다른 파일은 로드를 거부한다.
...
- **감독이 저장하는 선발은 정확히 11명(GK 1명), 벤치는 매치데이 명단, 배치 없음 =
  예비.** 팀 엔티티에 선발 배열은 없다 — `TACTIC_ASSIGNMENT`가 유일한 원본이다.
  코어의 자동 편성만은 열 명을 남길 수 있다 — 1군에 골키퍼가 없으면 골문을 비운다
  ([team.md](team.md) §6). 빈 자리는 저장되지 않으므로 그 사실은 등록 현황의
  사유로 선다.
