...
- `reason` — `"version"`이면 세이브 버전 불일치, `"corrupt"`면 파싱 실패거나 필수
  테이블 누락.
...
(위 [열 수 없는 세이브](#열-수-없는-세이브)). 목록 화면은
...
- **로드가 저장보다 열 배 비싸다.** 6MB 세이브 기준 `saveGame`은 28ms인데
  `loadGame`은 350ms이고, 그중 280ms가 `ensureSquadNumbers`다 — 미배정 선수마다
  전 선수를 훑어 O(n²)가 된다(5,743명 → 3,300만 회). 쓰기를 줄인 지금은 이쪽이
  왕복의 대부분이다.
