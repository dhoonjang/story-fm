...
읽기는 본 파일 → `.bak` 순으로 폴백한다.
목록 화면은 본문을 열지 않는다 — 저장할 때 요약을 `<id>.meta.json` 사이드카로
함께 쓰고, 파일 크기+mtime 지문으로 그게 지금 그 세이브의 요약인지 가린다.

**실패도 사이드카에 적는다.** 못 여는 세이브는 로드에 성공한 적이 없어 캐시가
영영 생기지 않았고, 목록을 열 때마다 수 MB를 다시 `JSON.parse`했다. 이제
`{ unreadable: { reason, saveVersion }, source: <지문> }`을 같은 자리에 쓴다.
지문이 파일 변경을 이미 감지하므로, 나중에 마이그레이션이 생겨 파일이 갱신되면
캐시가 저절로 무효가 되고 다시 시도된다.
