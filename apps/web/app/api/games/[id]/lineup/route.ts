...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  /**
   * 전술판 저장은 **전술판 선반의 타임라인**으로 남는다 (models.md §5) — 채팅 턴과 같은
   * 모양, 다른 선반: 무엇을 보냈고, 어느 명령이 걸리고 반려됐고, 판이 전후로 어떻게
  ...
  return withGameLock(id, LOCK_WAIT_MS.lineup, () =>
    traceBoard(id, async () => {
      ...
      noteTurn({
        input: { kind: "lineup", ...body.data, date: state.date, phase: state.phase },
        before: turnDigestOf(state),
      });
      /** 이 턴의 결과 — 반려도 결과다. 세이브에 앉은 것은 성공한 저장뿐이다 */
      const close = (ok: boolean, error?: string) =>
        noteTurn({
          outcome: { ok, saved: ok, ...(error === undefined ? {} : { error }) },
          after: turnDigestOf(state),
        });
      /** 전술판이 부르는 명령 — `wrap`을 지나지 않으므로 기록은 여기서 남긴다 */
      const command = (name: string, input: unknown, result: { ok: boolean; message: string }) =>
        journal({
          kind: "command",
          name,
          input,
      ...
      if (Object.keys(axes).length > 0) {
        ...
        command("set_tactics", axes, res);
        if (!res.ok) {
          close(false, res.message);
      ...
      const res = setLineup(state, lineupInput);
      command("set_lineup", lineupInput, res);
      if (!res.ok) {
        close(false, res.message);
        return NextResponse.json({ error: res.message }, { status: 400 });
      }
      ...
      for (const pick of body.data.roles ?? []) {
        ...
        command("set_player_role", pick, applied);
        if (!applied.ok) {
      ...
      if (takers && Object.keys(takers).length > 0) {
        ...
        command("set_set_piece_takers", takers, applied);
      ...
      if (routine && Object.keys(routine).length > 0) {
        ...
        command("set_set_piece_routine", routine, applied);
