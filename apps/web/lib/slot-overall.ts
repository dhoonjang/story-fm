 ...
 *
 * ⚠️ 안개는 **축에만** 있다(`AxisValues`가 이미 관측값이다) — 합성값은 전부
 * 거기서 파생되므로 자리를 어떻게 옮겨도 서로 어긋날 수 없다. 서버가 값마다
 * 따로 안개를 씌워 내려보내면(`overall`에 한 번, 자리별 전력에 또 한 번) 화면은
 * 참값이 없어 같은 규칙을 재현할 수 없고, 자리를 옮겨 보려면 "서버 값에 차이만
 * 얹는" 보정이 필요해진다 — 그 보정은 부르는 자리마다 갈려 같은 선수의 OVR이
 * 두 숫자로 보이게 된다.
 *
