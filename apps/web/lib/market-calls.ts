...
import { CARD_CALLS } from "./panel-hints";
 ...
 *
 * 코어가 이미 타입으로 보장한다(`MarketCommandResult`: 성공이면 카드가 반드시 있다).
 ...
 * 호출 결과를 **카드와 칩으로 가른다.**
 *
 * 가르는 기준은 payload의 모양이 아니라 **호출 이름**이다(`CARD_CALLS`) — 갈 장부가
 * 없는 호출만 카드로 선다. 모양으로 가르던 때는 카드를 실어야 할 호출이 payload를
 * 빠뜨려도 그냥 칩이 되어, 화면이 코어의 누락을 감췄다.
 *
 * `CARD_CALLS`인데 카드가 성하지 않으면 그 호출은 **어느 쪽에도 서지 않고** 콘솔에
