 ...
 * 전술 노트 — **줄이 그어진 한 장.**
 *
 * 판세가 접어 두는 것은 6축과 갈래가 남긴 이득·대가 여덟 줄이라(match.md §8), 표식도
 * 「적어 둔 것」의 꼴이어야 한다. 획은 다른 아이콘과 같은 1.7이고, 안쪽 줄은 셋이다 —
 * 15px에서 넷은 뭉치고 둘은 장이 비어 보인다.
 */
export function IconNote({ size = 15 }: IconProps) {
  return (
    <svg {...base(size)}>
      <path d="M5.6 4.2h12.8v15.6H5.6z" />
      <path d="M8.6 8.4h6.8" />
      <path d="M8.6 12h6.8" />
      <path d="M8.6 15.6h4.2" />
    </svg>
  );
}
