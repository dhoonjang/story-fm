 ...
 * 두었다. 보내는 것은 **문장이 아니라 구조체다**(`TurnOperation`) — 감독의 말투도
 * 아니고, 서버가 되읽을 문구도 아니다. 모델에 실릴 `<operator>시간 진행 — 하루</operator>`는
 * 서버가 그 구조체에서 만든다 (docs/llm/agents.md §2).
 *
...
const TIME_SKIPS = [
  ...
  { key: "week", label: "일주일", op: (): TurnOperation => ({ kind: "skip_days", days: 7 }) },
  {
    key: "match",
    label: "다음 경기",
    /**
     * **날짜를 붙여 보낸다.** 목표가 "다음 경기"라는 말뿐이면 서버가 일정을 다시
     * 찾아야 하는데, 클라이언트는 그 날짜를 이미 알고 있다(달력 뷰의 `isNext`).
     */
    op: (date: string): TurnOperation => ({ kind: "skip_to_next_match", date }),
  },
] as const;
...
export function Composer({
  ...
  /** 감독의 말을 보낸다 — 인자가 없으면 입력창의 글이 나간다 */
  onSend: (text?: string) => void;
  /** 손잡이를 누른다 — 감독의 발화가 아니라 조작이 나간다 */
