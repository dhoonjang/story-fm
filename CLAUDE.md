...
- **커밋/푸시**: 사용자가 요청할 때만. 작업 단위가 끝나면 지금 체크아웃된 브랜치에
  직접 커밋·푸시(`git push origin HEAD`). 공유 워크트리에선 경로를 짚어 add 하고
  rebase·stash·브랜치 전환은 하지 않는다 (AGENTS.md 5장).
...
> LLM 모드: `LLM_MODE=mock|real` (미지정 시 `config/llm.yml`의 GM 제공자 키가
> 있으면 real). 게임의 구조·상세 기획은 [docs/](./docs/README.md)가 단일 소스다 —
